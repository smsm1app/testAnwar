...
import {
  ...
  X, Receipt, Calendar, Wrench, ShieldAlert, Heart, ClipboardCheck, CreditCard, Archive
} from 'lucide-react';
...
function CustomersScreen({ permissions }: CustomersScreenProps) {
  ...
  const [activeMainTab, setActiveMainTab] = useState<'customers' | 'agents'>('customers');
  ...
  const [activeProfileTab, setActiveProfileTab] = useState<'info' | 'invoices' | 'installments' | 'maintenance' | 'installations' | 'debt' | 'archive'>('info');
  const [profileLoading, setProfileLoading] = useState(false);

  const [globalAgents, setGlobalAgents] = useState<any[]>([]);

  useEffect(() => {
    api.getAgents().then(res => setGlobalAgents(res || [])).catch(() => {});
  }, []);
  ...
  const handleOpenForm = (customer: any = null, autoFillAgentId: number | null = null) => {
    if (customer) {
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    toast(`⚠️ تحذير: هل أنت متأكد من حذف العميل "${name}" نهائياً؟ سيتم حذف جميع سجلاته (الفواتير، الأقساط، الحجوزات، الصيانة، العقود، الأرشيف) بشكل دائم ولا يمكن التراجع!`, {
      action: {
        label: '🗑️ حذف نهائي',
        onClick: async () => {
          try {
            ...
            toast.success('تم حذف العميل وجميع سجلاته المرتبطة بنجاح');
          } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-fade-in relative z-10 pb-12 max-w-7xl mx-auto">
      ...
      {/* Top Level Tabs */}
      <div className="flex gap-4 border-b border-white/50 pb-4 shrink-0 overflow-x-auto custom-scrollbar">
        <button 
          onClick={() => setActiveMainTab('customers')}
          className={`px-8 py-3 rounded-2xl font-black transition-all ${activeMainTab === 'customers' ? 'bg-amber-500 text-white shadow-lg' : 'bg-white/50 text-slate-600 hover:bg-white/80'}`}
        >
          العملاء
        </button>
        <button 
          onClick={() => setActiveMainTab('agents')}
          className={`px-8 py-3 rounded-2xl font-black transition-all ${activeMainTab === 'agents' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white/50 text-slate-600 hover:bg-white/80'}`}
        >
          الوكلاء
        </button>
      </div>
      ...
      {activeMainTab === 'agents' ? (
        <AgentsSegment permissions={permissions} onAddCustomer={(agentId: number) => handleOpenForm(null, agentId)} />
      ) : (
        <>
          ...
          <div className="w-[95%] max-w-2xl glass-card rounded-[2.5rem] shadow-2xl overflow-hidden animate-scale-up border border-white/80 max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar bg-white/50">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-bold text-slate-700 mb-2">الوكيل (اختياري)</label>
                  <select
                    value={formData.agentId || ''}
                    onChange={(e) => setFormData({ ...formData, agentId: e.target.value ? parseInt(e.target.value) : null })}
                    className="w-full bg-white/80 border border-white focus:ring-2 focus:ring-amber-500/50 rounded-2xl p-3.5 text-sm font-semibold text-slate-800 shadow-sm focus:outline-none transition-all"
                  >
                    <option value="">بدون وكيل (عميل مباشر)</option>
            ...
            {profileLoading || !profileData ? (
              ...
              <div className="flex-1 flex flex-col overflow-hidden bg-white/40">
                ...
                <div className="flex border-b border-white/60 bg-white/50 p-2 overflow-x-auto gap-2 shrink-0 custom-scrollbar">
                  ...
                  <button 
                    onClick={() => setActiveProfileTab('archive')}
                    className={`px-5 py-3 rounded-xl text-xs font-black flex items-center gap-2 transition-all duration-300 whitespace-nowrap ${
                      activeProfileTab === 'archive' ? 'bg-amber-500 text-white shadow-md shadow-amber-200' : 'text-slate-600 hover:text-slate-900 hover:bg-white'
                    }`}
                  >
                    <Archive className="w-4 h-4" /> الأرشيف الورقي ({profileData.archiveRecords?.length || 0})
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                  ...
                  {activeProfileTab === 'archive' && (
                    <div className="space-y-6 animate-fade-in">
                      <div className="border-b border-white/50 pb-4">
                        <h4 className="font-black text-slate-800 text-base">الأرشيف الورقي (سجلات تاريخية)</h4>
                        <p className="text-xs text-slate-500 mt-1 font-medium">المنظومات والأعمال التي تم تسجيلها قديماً قبل التحول الرقمي.</p>
                      </div>

                      {(profileData.archiveRecords || []).length === 0 ? (
                        <div className="text-center py-12 text-slate-400 text-sm font-bold bg-white/40 rounded-[2rem] border border-white shadow-sm">
                          لا توجد سجلات أرشيف ورقي لهذا المشترك.
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {(profileData.archiveRecords || []).map((record: any) => (
                            <div key={record.id} className="bg-white/60 border border-white p-6 rounded-[2rem] shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden">
                              <div className="absolute top-0 inset-x-0 h-1.5 bg-gradient-to-r from-amber-400 to-orange-500 opacity-70"></div>
                              
                              <div className="flex justify-between items-start border-b border-white/60 pb-4 mb-4 mt-2">
                                <div>
                                  <h5 className="font-black text-slate-800 text-base flex items-center gap-2">
                                    <Archive className="w-4 h-4 text-amber-500" />
                                    سجل أرشيف رقم: {record.id}
                                  </h5>
                                  <p className="text-xs text-slate-500 font-mono mt-1">تاريخ التنصيب: {record.installationDate || 'غير متوفر'}</p>
                                </div>
                                <span className="bg-amber-50 text-amber-700 px-3 py-1 rounded-xl text-[10px] font-black border border-amber-200">
                                  من الأرشيف
                                </span>
                              </div>
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-100">
                                  <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">حجم المنظومة</span>
                                  <span className="font-bold text-slate-800">{record.systemSize || '---'}</span>
                                </div>
                                <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-100">
                                  <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">الانفيرتر</span>
                                  <span className="font-bold text-slate-800">{record.inverterSize || '---'}</span>
                                </div>
                                <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-100">
                                  <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">عدد البطاريات</span>
                                  <span className="font-bold text-slate-800">{record.batteriesCount || '---'}</span>
                                </div>
                                <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-100">
                                  <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">عدد الألواح</span>
                                  <span className="font-bold text-slate-800">{record.panelsCount || '---'}</span>
                                </div>
                              </div>
                              
                              {record.notes && (
                                <div className="mt-4 text-xs text-slate-500 bg-amber-50/50 p-3 rounded-xl border border-amber-100 italic">
                                  ملاحظات الأرشيف: {record.notes}
                                </div>
                              )}
                            </div>
                          ))}
