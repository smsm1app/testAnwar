...
export default function ProductsScreen({ permissions }: ProductsScreenProps) {
  ...
  const handleNumberInput = (val: string) => {
    if (!val) return '';
    return val.replace(/[^0-9\u0660-\u0669\u06F0-\u06F9.-]/g, '')
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    if (!finalSku) {
      finalSku = (isSystemProduct ? 'SYS-' : 'PRD-') + Math.random().toString(36).substring(2, 8).toUpperCase();
    }

    const finalPurchasePrice = productForm.purchasePrice;
    const finalSellingPrice = productForm.sellingPrice;

    if (!productForm.name || !finalCategoryId || !finalPurchasePrice || !finalSellingPrice) {
                  ...
                  />
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        inputMode="numeric"
                        value={productForm.purchasePrice ? Number(productForm.purchasePrice).toLocaleString('en-US') : ''}
                        onChange={(e) => { const val = e.target.value.replace(/,/g, ''); setProductForm({ ...productForm, purchasePrice: handleNumberInput(val) }); }}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        inputMode="numeric"
                        value={productForm.sellingPrice ? Number(productForm.sellingPrice).toLocaleString('en-US') : ''}
                        onChange={(e) => { const val = e.target.value.replace(/,/g, ''); setProductForm({ ...productForm, sellingPrice: handleNumberInput(val) }); }}
                ...
                {!isSystemProduct && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setProductForm({ ...productForm, quantity: handleNumberInput(e.target.value) })}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-end bg-white/80 p-5 rounded-2xl border border-slate-100 shadow-sm">
                      ...
                      <div className="w-full sm:w-32">
                        ...
                        <input
                          ...
                          onChange={(e) => setSelectedComponentQty(handleNumberInput(e.target.value))}
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setProductForm({ ...productForm, minStockAlert: handleNumberInput(e.target.value) })}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setProductForm({ ...productForm, warrantyMonths: handleNumberInput(e.target.value) })}
                        className="w-full bg-white border border-slate-200 rounded-xl p-3.5 font-mono font-bold focus:ring-2 focus:ring-amber-500 focus:outline-none shadow-sm transition-all"
                        placeholder="12"
                      />
                    </div>
