...
export default function WorkersScreen({ permissions, currentUser }: WorkersScreenProps) {
  ...
  const workerFinancialData = React.useMemo(() => {
    ...
    // Get all payments for this worker for COMPLETED tasks only
    const wPayments = workerPayments.filter(p => {
      const isWorkerMatch = p.worker_id === viewingWorkerStats.id || p.worker_name === viewingWorkerStats.name || p.workerId === viewingWorkerStats.id || p.workerName === viewingWorkerStats.name;
      if (!isWorkerMatch) return false;
      
      const taskId = String(p.task_id || p.taskId);
      const taskType = p.task_type || p.taskType;
      
      if (taskType === 'booking') {
        const b = bookings.find(b => String(b.id) === taskId);
        return b && b.status === 'completed';
      }
      if (taskType === 'maintenance') {
        const m = maintenance.find(m => String(m.id) === taskId);
        return m && (m.status === 'repaired' || m.status === 'closed');
      }
      if (taskType === 'fault') {
        const f = faults.find(f => String(f.id) === taskId);
        return f && (f.status === 'repaired' || f.status === 'closed');
      }
      return true;
    });
  ...
  }, [viewingWorkerStats, workerPayments, workerSettlements, bookings, maintenance, faults]);
  ...
  const workerStatsData = React.useMemo(() => {
    ...
    const completedInstallations = bookings.filter(b => {
      ...
      // Hide if the booking was made before the reset, use createdAt to avoid future appointmentDate bugs
      const taskDate = new Date(b.createdAt || b.appointmentDate || 0).setHours(0,0,0,0);
  ...
  const getWorkerQuickSummary = (worker: any) => {
    const wPay = workerPayments.filter(p => {
      const isWorkerMatch = p.worker_id === worker.id || p.worker_name === worker.name || p.workerId === worker.id || p.workerName === worker.name;
      if (!isWorkerMatch) return false;
