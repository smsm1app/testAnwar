...
export default function EmployeeAccountingScreen({ permissions }: { permissions: any }) {
  ...
  // ── Excel export ──────────────────────────────────────────────────────────
  const handleExport = () => {
    const rows = transactions.map(t => [t.employeeName, TX_TYPES[t.type]?.label || t.type, t.amount, t.date, t.notes].join('\t'));
    const content = ['الموظف\tالنوع\tالمبلغ\tالتاريخ\tملاحظات', ...rows].join('\n');
