                        ...
                        <tr key={adv.id} className="hover:bg-white/60 transition-colors">
                          ...
                          <td className="p-4 text-center">
                            {adv.status === 'completed' ? (
                              <span className="inline-block whitespace-nowrap bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 py-1 rounded-lg text-xs font-bold">مكتملة ✓</span>
                            ) : isOverdue ? (
                              <span className="inline-block whitespace-nowrap bg-rose-50 text-rose-700 border border-rose-200 px-3 py-1 rounded-lg text-xs font-bold">تأخرت!</span>
                            ) : (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField label="الراتب الشهري (د.ع)">
                <input type="text" inputMode="numeric" value={empForm.monthlySalary ? Number(empForm.monthlySalary).toLocaleString('en-US') : ''} onChange={e => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setEmpForm(p => ({ ...p, monthlySalary: val })); }} placeholder="0" className="input-field" style={{ direction: 'ltr' }} />
              </FormField>
              <FormField label="الأجر اليومي (د.ع)">
                <input type="text" inputMode="numeric" value={empForm.dailyWage ? Number(empForm.dailyWage).toLocaleString('en-US') : ''} onChange={e => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setEmpForm(p => ({ ...p, dailyWage: val })); }} placeholder="0" className="input-field" style={{ direction: 'ltr' }} />
              </FormField>
            </div>
      ...
      {showAdvForm && (
        <Modal title="إضافة سلفة" onClose={() => setShowAdvForm(false)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="المبلغ *"><input type="text" inputMode="numeric" value={advForm.amount ? Number(advForm.amount).toLocaleString('en-US') : ''} onChange={e => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setAdvForm(p => ({ ...p, amount: val })); }} placeholder="0" className="input-field" style={{ direction: 'ltr' }} /></FormField>
        ...
        <Modal title="تسديد سلفة يدوياً" onClose={() => setShowRepayForm(false)}>
          <div className="space-y-4">
            ...
            <FormField label="المبلغ المراد تسديده الآن (د.ع) *">
              <input type="text" inputMode="numeric" value={repayAmount ? Number(repayAmount).toLocaleString('en-US') : ''} onChange={e => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setRepayAmount(val); }} placeholder="مثال: 50,000" className="input-field text-xl font-bold font-mono text-center" style={{ direction: 'ltr' }} />
            </FormField>
        ...
        <Modal title="إضافة معاملة مالية" onClose={() => setShowTxForm(false)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField label="المبلغ *"><input type="text" inputMode="numeric" value={txForm.amount ? Number(txForm.amount).toLocaleString('en-US') : ''} onChange={e => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setTxForm(p => ({ ...p, amount: val })); }} placeholder="0" className="input-field" style={{ direction: 'ltr' }} /></FormField>
            </div>
