...
import {
  ...
  Upload, CheckSquare, RefreshCcw, Landmark, Clock, MessageSquareShare, Edit, Trash2, Sparkles
} from 'lucide-react';
...
const calculateInvoiceProfit = (inv: any) => {
  ...
  return finalAmt - totalCost;
};

export default function InvoicesScreen({ permissions }: InvoicesScreenProps) {
  const canShowProfitMargin = Boolean(permissions?.invoices?.showProfitMargin || permissions?.sales?.showProfitMargin);
        ...
        <div className="glass-card rounded-[2rem] border border-white/80 shadow-xl overflow-hidden">
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-right text-xs">
              <thead>
                <tr className="bg-white/50 text-slate-500 border-b border-white">
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide whitespace-nowrap">الكود</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide">العميل</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide whitespace-nowrap">التاريخ</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide">النوع</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide whitespace-nowrap">المبلغ</th>
                  {canShowProfitMargin && <th className="px-3 py-2.5 font-black uppercase tracking-wide text-teal-700 whitespace-nowrap">الربح</th>}
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide whitespace-nowrap">المتبقي</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide">الحالة</th>
                  <th className="px-3 py-2.5 font-black uppercase tracking-wide text-left whitespace-nowrap">الأدوات</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/40">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-white/60 transition-colors duration-200 group">
                    <td className="px-3 py-2.5 font-mono font-bold text-slate-700 text-xs whitespace-nowrap">{inv.invoiceNumber}</td>
                    <td className="px-3 py-2.5 font-bold text-slate-800 text-xs max-w-[140px] truncate" title={inv.customerName}>{inv.customerName}</td>
                    <td className="px-3 py-2.5 font-mono text-slate-500 text-xs whitespace-nowrap">{inv.date}</td>
                    <td className="px-3 py-2.5">
                      {inv.invoiceType === 'cash' && <span className="bg-emerald-100 text-emerald-800 border border-emerald-200 px-2 py-0.5 rounded-lg font-bold text-[11px] whitespace-nowrap">نقدي</span>}
                      {inv.invoiceType === 'partial' && <span className="bg-blue-100 text-blue-800 border border-blue-200 px-2 py-0.5 rounded-lg font-bold text-[11px] whitespace-nowrap">ذمم</span>}
                      {inv.invoiceType === 'installment' && <span className="bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-lg font-bold text-[11px] whitespace-nowrap">أقساط</span>}
                      {inv.invoiceType === 'mastercard' && <span className="bg-purple-100 text-purple-800 border border-purple-200 px-2 py-0.5 rounded-lg font-bold text-[11px] whitespace-nowrap">كارد</span>}
                    </td>
                    <td className="px-3 py-2.5 font-bold font-mono text-slate-900 text-xs whitespace-nowrap">{formatIQD(inv.finalAmount)}</td>
                    {canShowProfitMargin && (
                      <td className="px-3 py-2.5 font-mono font-bold text-teal-700 text-xs whitespace-nowrap">
                        <span className="inline-block whitespace-nowrap text-teal-700 bg-teal-50 px-2 py-0.5 rounded-lg border border-teal-100">
                          {formatIQD(calculateInvoiceProfit(inv))}
                        </span>
                      </td>
                    )}
                    <td className="px-3 py-2.5 font-mono font-bold text-xs whitespace-nowrap">
                      {inv.remainingAmount > 0 ? (
                        <span className="inline-block whitespace-nowrap text-rose-600 bg-rose-50 px-2 py-0.5 rounded-lg border border-rose-100">{formatIQD(inv.remainingAmount)}</span>
                      ) : (
                        <span className="text-slate-400">-</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      <span className={`px-2 py-0.5 rounded-lg text-[11px] font-black border ${inv.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                        {inv.status === 'active' ? 'سليم' : 'ملغى'}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-1.5 opacity-70 group-hover:opacity-100 transition-opacity">
                        {inv.invoiceType === 'partial' && inv.remainingAmount > 0 && inv.status === 'active' && (
                          <button
                            ...
                            title="تسديد دفعة"
                            className="px-2 py-1 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-lg text-[11px] font-black transition-all whitespace-nowrap"
                          >
                            دفعة
                          </button>
                        )}
                        <button
                          ...
                          className="px-2 py-1 bg-slate-100 hover:bg-white text-slate-700 rounded-lg text-[11px] font-black border border-transparent hover:border-slate-200 transition-all whitespace-nowrap"
                        >
                          عرض
                        </button>
                        <button
                          ...
                          className="p-1.5 bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white transition-all rounded-lg border border-emerald-100"
                        >
                        ...
                        {permissions?.invoices?.edit && inv.status === 'active' && (
                          <button
                            ...
                            className="p-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-lg border border-amber-100 transition-all"
                          >
                        ...
                        {(permissions?.invoices?.delete || permissions?.sales?.delete) && (
                          <button
                            ...
                            className="p-1.5 bg-rose-50 hover:bg-rose-500 text-rose-600 hover:text-white rounded-lg border border-rose-100 transition-all"
                          >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
          <div className="w-[95%] max-w-3xl glass-card rounded-[2.5rem] shadow-2xl overflow-hidden animate-scale-up border border-white/80 flex flex-col justify-between max-h-[90vh]">
            ...
            <div id="invoice-printable-canvas" className="flex-1 overflow-y-auto custom-scrollbar p-8 sm:p-10 bg-white print:p-8 print:bg-white text-slate-900 flex flex-col min-h-full relative print:block">
              ...
              <div className="mb-8 relative z-10 min-h-[250px] shrink-0">
              ...
              {/* Totals Section */}
              <div className="flex justify-between items-start mb-12 relative z-10 mt-auto pt-8 shrink-0">
                ...
                <div className="w-96 bg-white border-2 border-slate-900 rounded-xl overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    <tbody className="divide-y divide-slate-100">
                      ...
                      {canShowProfitMargin && (
                        <tr className="bg-teal-50 border-t-2 border-teal-300 print:hidden">
                          <td className="py-3 px-4 text-teal-900 font-black flex items-center gap-1.5">
                            <Sparkles className="w-4 h-4 text-teal-600" />
                            هامش الربح الإداري:
                          </td>
                          <td className="py-3 px-4 text-left font-mono font-black text-teal-900 text-base">
                            {formatIQD(calculateInvoiceProfit(selectedInvoice))}
                          </td>
                        </tr>
                      )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input
                        type="text"
                        inputMode="numeric"
                        value={newPartialAmount ? Number(newPartialAmount).toLocaleString('en-US') : ''}
                        onChange={(e) => { const val = e.target.value.replace(/,/g, ''); if (!isNaN(Number(val)) || val === '') setNewPartialAmount(val); }}
