...
export default function POSScreen({ permissions, user, onInvoiceCreated, isDarkTheme = false }: POSScreenProps) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [productSearch, posMode, selectedCategory]);
  ...
  const loadPOSData = async (silent = false) => {
    try {
      ...
      const custRes = await api.getCustomers(1, 1000).catch(() => []);
  ...
  const handleProcessSale = async () => {
    ...
    if ((invoiceType === 'installment' || invoiceType === 'partial' || invoiceType === 'mastercard') && downPayment >= finalPayable) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 animate-fade-in relative z-10 max-w-screen-2xl mx-auto pb-12 h-full">
      ...
      <div className="lg:col-span-7 glass-card rounded-[2.5rem] p-5 md:p-8 shadow-xl flex flex-col h-[calc(100vh-6rem)]">
        ...
        {/* Category Filter */}
        {categories.length > 0 && (
          <div className="flex gap-2 overflow-x-auto custom-scrollbar mb-6 shrink-0 bg-white/40 p-2 rounded-2xl border border-white/80 shadow-inner">
            <button
            ...
            {(() => {
              ...
              return (
                ...
                return (
                  ...
                  {totalPages > 1 && (
                    <div className="col-span-full flex justify-center items-center gap-3 mt-6 pt-6 border-t border-white/50">
                      <button
                        onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                        disabled={currentPage === 1}
                        className="px-5 py-2.5 bg-white/80 hover:bg-white rounded-xl text-sm font-black text-slate-700 disabled:opacity-50 transition-all border border-white shadow-sm"
                      >
                        السابق
                      </button>
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-10 h-10 flex items-center justify-center rounded-xl text-sm font-black transition-all shadow-sm border ${currentPage === pageNum ? 'bg-amber-500 text-white border-amber-500 shadow-amber-200' : 'bg-white/60 hover:bg-white text-slate-600 border-white/80'}`}
                            >
                              {pageNum}
                            </button>
                          );
      ...
      <div className="lg:col-span-5 flex flex-col gap-6 h-[calc(100vh-6rem)] relative z-10">
        ...
        <div className="glass-card rounded-[2.5rem] p-5 md:p-8 shadow-xl flex flex-col flex-1 overflow-y-auto custom-scrollbar justify-between border border-white/80">
          ...
          <div className="shrink-0 mt-5 space-y-4 text-sm bg-white/60 p-6 rounded-3xl border border-white shadow-sm font-medium">
            ...
            {/* Down Payment entry for installments, partial, and mastercard */}
            {(invoiceType === 'installment' || invoiceType === 'partial' || invoiceType === 'mastercard') && (
              <div className="flex items-center justify-between animate-fade-in border-t border-slate-100 pt-4 mt-2">
                <span className="flex items-center gap-2 text-slate-700 font-bold">
                  ...
                  {invoiceType === 'partial' ? 'الدفعة المقبوضة (نقداً):' : invoiceType === 'mastercard' ? 'الدفعة المقدمة (نقداً):' : 'المقدمة (نقداً):'}
                </span>
                <input
                  type="text"
            ...
            <div className="pt-5 border-t border-white/50 space-y-3">
              ...
              {invoiceType === 'mastercard' && (
                <div className="flex justify-between items-center bg-purple-50/80 p-4 rounded-2xl border border-purple-200 mt-3 shadow-sm">
                  <span className="text-purple-700 font-black">الصافي الذي سيُرحل لتسوية الماستركارد:</span>
                  <span className="font-mono font-black text-purple-800 text-lg tracking-tighter">{formatIQD(Math.max(0, finalPayable - downPayment))}</span>
                </div>
              )}
              ...
              {/* Profit Indicator - visible to admin or users with showProfitMargin permission */}
              {(permissions?.sales?.showProfitMargin || permissions?.invoices?.showProfitMargin || user?.position === 'مدير النظام') && (
