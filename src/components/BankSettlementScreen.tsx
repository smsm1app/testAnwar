...
export default function BankSettlementScreen({ permissions, user }: BankSettlementScreenProps) {
  ...
  const totalMasterCardAmount = invoices.reduce((acc, inv) => acc + ((parseFloat(inv.finalAmount) || 0) - (parseFloat(inv.posCashAdvance) || 0)), 0);
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-6 py-4 font-mono font-bold text-indigo-700">{formatIQD((parseFloat(inv.finalAmount) || 0) - (parseFloat(inv.posCashAdvance) || 0))}</td>
