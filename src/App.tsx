...
const WorkersScreen = React.lazy(() => import('./components/WorkersScreen'));
...
const EmployeeAccountingScreen = React.lazy(() => import('./components/EmployeeAccountingScreen'));
const ArchiveScreen = React.lazy(() => import('./components/ArchiveScreen'));
const MoneyTransfersScreen = React.lazy(() => import('./components/MoneyTransfersScreen'));

import {
  BarChart3, Users, Package, ShoppingCart, Receipt, Calendar,
  Wrench, ShieldCheck, TrendingUp, Settings, LogOut, Moon, Sun,
  Bell, Building2, UserCircle, AlignRight, X, Landmark, FileSignature, DollarSign, Archive, Banknote
} from 'lucide-react';
...
export default function App() {
  const injectPermissions = (parsedUser: any) => {
    ...
    // Auto-inject employeeAccounting permission if missing (give to employees viewers)
    if (!parsedUser.permissions.employeeAccounting) {
      const hasEmp = parsedUser.permissions.employees?.view || false;
      parsedUser.permissions.employeeAccounting = {
        view: hasEmp, create: hasEmp, edit: hasEmp, delete: hasEmp, approve: hasEmp, export: hasEmp, viewWidget: hasEmp
      };
    }
    // Auto-inject moneyTransfers permission if missing
    if (!parsedUser.permissions.moneyTransfers) {
      parsedUser.permissions.moneyTransfers = {
        view: false, create: false, edit: false, delete: false, approve: false, export: false, viewWidget: false
      };
    }
    // Auto-inject workersManagement permission if missing
    if (!parsedUser.permissions.workersManagement) {
      const hasInst = parsedUser.permissions.installationTeams?.view || parsedUser.permissions.installationBookings?.view || false;
      parsedUser.permissions.workersManagement = {
        view: hasInst, create: hasInst, edit: hasInst, delete: hasInst, approve: hasInst, export: hasInst, viewWidget: hasInst
      };
    }
    ...
    // Auto-inject archive permission if missing
    if (!parsedUser.permissions.archive) {
      const hasMnt = parsedUser.permissions.maintenance?.view || false;
      parsedUser.permissions.archive = {
        view: hasMnt, create: hasMnt, edit: hasMnt, delete: hasMnt, approve: hasMnt, export: hasMnt, viewWidget: hasMnt
      };
    }
  ...
  const checkSession = async () => {
    ...
    try {
      ...
      const injected = injectPermissions(rawUser);
      localStorage.setItem('user', JSON.stringify(injected));
      setUser(injected);
    } catch (err) {
      // Stale token
  ...
  useEffect(() => {
    if (user && user.permissions) {
      ...
      const accessMap: Record<string, boolean> = {
        ...
        workers: !!perms?.workersManagement?.view,
        employees: !!perms?.employees?.view,
        employeeAccounting: !!perms?.employeeAccounting?.view,
        moneyTransfers: !!perms?.moneyTransfers?.view,
  ...
  const menuItems = [
    ...
    { id: 'archive', name: 'الأرشيف', icon: Archive, show: !!permissions?.archive?.view },
    ...
    { id: 'workers', name: 'إدارة العمال', icon: Users, show: permissions?.workersManagement?.view },
    { id: 'employees', name: 'الموظفين والصلاحيات', icon: ShieldCheck, show: permissions?.employees?.view },
    { id: 'employeeAccounting', name: 'محاسبة الموظفين', icon: DollarSign, show: permissions?.employeeAccounting?.view },
    { id: 'moneyTransfers', name: 'الحوالات والصيرفات', icon: Banknote, show: permissions?.moneyTransfers?.view },
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pr-64">
        ...
        <main className="p-4 md:p-6 lg:p-8 flex-1 overflow-y-auto max-w-7xl w-full mx-auto relative">
          <React.Suspense fallback={
            ...
            { activeTab === 'archive' && permissions?.archive?.view && <ArchiveScreen permissions={permissions} /> }
            ...
            {activeTab === 'workers' && permissions?.workersManagement?.view && <WorkersScreen permissions={permissions} currentUser={user} />}
            {activeTab === 'employees' && permissions?.employees?.view && <EmployeesScreen permissions={permissions} currentUser={user} />}
            {activeTab === 'employeeAccounting' && permissions?.employeeAccounting?.view && <EmployeeAccountingScreen permissions={permissions} />}
            {activeTab === 'moneyTransfers' && permissions?.moneyTransfers?.view && <MoneyTransfersScreen permissions={permissions} />}
