...
const logAudit = async (user: string, action: string, affectedRecord: string, ip: string) => {
  try {
    ...
    if (error) {
      console.error('[AUDIT LOG ERROR]', action, error);
      try {
        fs.appendFileSync('audit_error.log', new Date().toISOString() + ' ' + action + ' ' + JSON.stringify(error) + '\n');
      } catch (fsErr) {
        console.error('[AUDIT LOG FS ERROR]', fsErr);
      }
    }
  } catch (err) {
    console.error('[AUDIT LOG EXCEPTION]', action, err);
    try {
      fs.appendFileSync('audit_error.log', new Date().toISOString() + ' ' + action + ' ' + JSON.stringify(err) + '\n');
    } catch (fsErr) {
      console.error('[AUDIT LOG FS ERROR]', fsErr);
    }
  }
...
const normalizePermissions = (rawPerm: any) => {
  const defaultActions = { view: false, create: false, edit: false, delete: false, approve: false, export: false, showProfitMargin: false };
  const ALL_KEYS: PermissionKey[] = [
    ...
    'reports', 'employees', 'settings', 'auditLogs', 'backups', 'contracts', 'employeeAccounting', 'moneyTransfers'
  ];
  ...
  if (rawPerm) {
    ...
    // Auto-inject employeeAccounting for admins (employees.view implies access)
    if (!rawPerm.employeeAccounting && rawPerm.employees?.view) {
      normalized.employeeAccounting = { ...defaultActions, view: true, create: true, edit: true, delete: true, approve: true, export: true };
    }
  }
...
app.delete('/api/installation-workers/:id', authenticate, async (req: any, res) => {
  ...
  // Remove worker from all teams
  const { data: teams } = await supabase.from('installation_teams').select('*');
  if (teams) {
    for (const team of teams) {
      let updated = false;
      let newMembers = team.members;
      
      if (Array.isArray(newMembers) && newMembers.includes(worker.name)) {
        newMembers = newMembers.filter((m: any) => m !== worker.name);
        updated = true;
      } else if (typeof newMembers === 'string' && newMembers.includes(worker.name)) {
        newMembers = newMembers.split(/[,،\n\r]+/).map((m: string) => m.trim()).filter(Boolean).filter((m: string) => m !== worker.name);
        updated = true;
      }

      let newLeader = team.leader;
      if (team.leader === worker.name) {
        const memArray = Array.isArray(newMembers) ? newMembers : (typeof newMembers === 'string' ? newMembers.split(/[,،\n\r]+/).map((m: string) => m.trim()).filter(Boolean) : []);
        newLeader = memArray[0] || 'مسؤول طاقم';
        updated = true;
      }
      
      if (updated) {
        await supabase.from('installation_teams').update({ members: newMembers, leader: newLeader }).eq('id', team.id);
      }
    }
  }
...
app.post('/api/installation-workers/:id/reset', authenticate, async (req: any, res) => {
  // We can use the worker settlement approve permission or delete permission to authorize this
  if (!req.user.permissions.workerSettlement?.approve && !req.user.permissions.installationTeams?.delete) {
    return res.status(403).json({ error: 'ليس لديك صلاحية لتصفير حسابات العمال' });
  }
  const id = parseInt(req.params.id);
  ...
  // Delete all financial records for this worker
  const { error: err1 } = await supabase.from('worker_payments').delete().eq('worker_id', id);
  if (err1) return res.status(500).json({ error: err1.message });

  const { error: err2 } = await supabase.from('worker_settlements').delete().eq('workerId', id);
  if (err2) return res.status(500).json({ error: err2.message });

  // Insert a marker to record the reset date so we can hide older tasks in the UI
  await supabase.from('worker_settlements').insert({
    workerId: id,
    taskId: 0,
    taskType: 'reset_marker',
    amount: 0,
    settledAt: new Date().toISOString(),
    settledBy: req.user.name
  });

  await logAudit(req.user.name, 'تصفير حساب عامل', worker.name, getClientIp(req));
  res.json({ success: true });
});
...
// Record a partial settlement payment for a worker
app.post('/api/worker-settlements/pay', authenticate, async (req: any, res) => {
  const { workerId, amount } = req.body;
  if (!workerId || !amount || parseFloat(amount) <= 0) {
    return res.status(400).json({ error: 'يرجى إدخال معرف العامل والمبلغ بشكل صحيح' });
  }

  const { data, error } = await supabase.from('worker_settlements').insert({
    workerId: workerId,
    taskId: 0,
    taskType: 'payment',
  ...
  await logAudit(req.user.name, 'محاسبة عامل (دفع)', `عامل: ${workerId}, المبلغ: ${amount}`, getClientIp(req));
  res.status(201).json(data);
});

// ==================== WORKER PAYMENTS (SUPABASE) ====================

app.get('/api/worker-payments', authenticate, async (req: any, res) => {
  const { data, error } = await supabase.from('worker_payments').select('*').order('id', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data || []);
});

app.post('/api/worker-payments/bulk', authenticate, async (req: any, res) => {
  const { payments } = req.body;
  if (!payments || !Array.isArray(payments) || payments.length === 0) {
    return res.status(400).json({ error: 'يرجى إرسال بيانات الأجور' });
  }
  ...
  const { data, error } = await supabase.from('worker_payments').insert(rows).select();
  if (error) return res.status(500).json({ error: error.message });

  await logAudit(req.user.name, 'تسجيل أجور عمال', `عدد: ${rows.length}`, getClientIp(req));
  res.status(201).json(data || []);
});

app.post('/api/worker-payments/task', authenticate, async (req: any, res) => {
  const { taskId, taskType = 'booking', payments = [] } = req.body;
  if (!taskId) {
    return res.status(400).json({ error: 'رقم المهمة مطلوب' });
  }

  // Delete old payment records for this task
  await supabase.from('worker_payments').delete().eq('task_id', taskId).eq('task_type', taskType);

  if (!Array.isArray(payments) || payments.length === 0) {
    await logAudit(req.user.name, 'تعديل أجور عمال المهمة', `مهمة: ${taskId}, تفريغ الأجور`, getClientIp(req));
    return res.json({ success: true, count: 0 });
  }

  const rows = payments
    .filter((p: any) => (parseFloat(p.amount) > 0 || p.isPaidInAdvance))
    .map((p: any) => ({
      worker_id: p.workerId || 0,
      worker_name: p.workerName || '',
      task_id: taskId,
      task_type: taskType,
      amount: parseFloat(p.amount) || 0,
  ...
  if (rows.length > 0) {
    const { data, error } = await supabase.from('worker_payments').insert(rows).select();
    if (error) return res.status(500).json({ error: error.message });
    await logAudit(req.user.name, 'تعديل أجور عمال المهمة', `مهمة: ${taskId}, عدد: ${rows.length}`, getClientIp(req));
...
app.get('/api/agents', authenticate, async (req: any, res) => {
  if (!req.user.permissions.customers.view) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { data, error } = await supabase.from('agents').select('*').eq('is_deleted', false).order('name', { ascending: true });
  if (error) return res.status(500).json({ error: error.message });
  
  const agentsWithTotals = await Promise.all(data.map(async (agent: any) => {
    const { data: customers } = await supabase.from('customers').select('id, name').eq('agent_id', agent.id).eq('is_deleted', false);
    
    const customersWithDebt = await Promise.all((customers || []).map(async (cust: any) => {
      const { data: invoices } = await supabase.from('invoices').select('remaining_amount').eq('customer_id', cust.id).eq('status', 'active');
      const debt = (invoices || []).reduce((acc, inv) => acc + (parseFloat(inv.remaining_amount) || 0), 0);
      return { ...cust, totalUnpaid: debt };
    }));

    const totalUnpaid = customersWithDebt.reduce((sum, c) => sum + c.totalUnpaid, 0);
  ...
  res.json(agentsWithTotals);
});

app.post('/api/agents', authenticate, async (req: any, res) => {
  if (!req.user.permissions.customers.create) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { name, phone } = req.body;
  if (!name) return res.status(400).json({ error: 'الاسم مطلوب' });
  
  const { data, error } = await supabase.from('agents').insert({ name, phone }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'إضافة وكيل', `${name}`, getClientIp(req));
  res.status(201).json({ ...data, customers: [], totalUnpaid: 0 });
});

app.put('/api/agents/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.customers.edit) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { name, phone } = req.body;
  const updates: any = {};
  if (name) updates.name = name;
  if (phone !== undefined) updates.phone = phone;
  
  const { data, error } = await supabase.from('agents').update(updates).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'تعديل وكيل', `${name}`, getClientIp(req));
  res.json(data);
});

app.delete('/api/agents/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.customers.delete) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { error } = await supabase.from('agents').update({ is_deleted: true }).eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'حذف وكيل', `معرف ${req.params.id}`, getClientIp(req));
  res.json({ success: true });
});

app.post('/api/agents/:id/pay', authenticate, async (req: any, res) => {
  if (!req.user.permissions.invoices.create) return res.status(403).json({ error: 'لا تملك الصلاحية' });
  const { payments } = req.body; 
  if (!payments || !Array.isArray(payments) || payments.length === 0) return res.status(400).json({ error: 'لا توجد دفعات صالحة' });
  
  const { date: payDate } = getNowDateTimeString();
  
  for (const pay of payments) {
    let remainingAmount = parseFloat(pay.amount);
    if (remainingAmount <= 0) continue;
    
    const { data: unpaidInvoices } = await supabase.from('invoices')
      .select('*').eq('customer_id', pay.customerId).eq('status', 'active').gt('remaining_amount', 0).order('created_at', { ascending: true });
    ...
    for (const inv of unpaidInvoices) {
      ...
      const payNow = Math.min(remainingAmount, invRemaining);
      const newRemaining = invRemaining - payNow;
      
      const { error: insErr } = await supabase.from('partial_payments').insert({
        invoice_id: inv.id,
        amount: payNow,
        notes: `دفعة مجمعة من الوكيل`,
      ...
      if (insErr) {
        console.error('Error inserting partial payment:', insErr);
        continue;
      }
      
      await supabase.from('invoices').update({ remaining_amount: newRemaining }).eq('id', inv.id);
      
      remainingAmount -= payNow;
    }
  }
  
  await logAudit(req.user.name, 'تسديد دفعة وكيل', `تم تسديد الدفعات`, getClientIp(req));
  res.json({ success: true });
});

app.get('/api/agents/:id/payments', authenticate, async (req: any, res) => {
  if (!req.user.permissions.invoices.view) return res.status(403).json({ error: 'لا تملك الصلاحية' });
  const { data, error } = await supabase
    .from('partial_payments')
    .select(`id, amount, date, notes, "user", invoices ( invoice_number, customer_name )`)
    .eq('agent_id', parseInt(req.params.id))
...
app.get('/api/customers', authenticate, async (req: any, res) => {
  ...
  if (search) {
    if (!isNaN(Number(search))) {
      query = query.or(`name.ilike.%${search}%,phone.ilike.%${search}%,id.eq.${Number(search)}`);
    } else {
  ...
  const customersWithTotals = await Promise.all(data.map(async (cust: any) => {
    const { data: invoices } = await supabase.from('invoices').select('remaining_amount').eq('customer_id', cust.id).eq('status', 'active');
    const debt = (invoices || []).reduce((acc, inv) => acc + (parseFloat(inv.remaining_amount) || 0), 0);
    return { ...cust, total_unpaid: debt };
  }));
  
  res.json({
    data: customersWithTotals.map(mapCustomer),
    total: count || 0,
    page,
...
app.post('/api/customers', authenticate, async (req: any, res) => {
  ...
  const { name, phone, secondaryPhone, address, mapsLink, gpsCoords, notes, agentId } = req.body;
  if (!name || !phone || !address) return res.status(400).json({ error: 'الاسم والهاتف والعنوان مطلوبة' });
...
app.delete('/api/customers/:id', authenticate, async (req: any, res) => {
  ...
  // Fetch customer name for audit log before deletion
  const { data: customer } = await supabase.from('customers').select('name').eq('id', id).single();
  if (!customer) return res.status(404).json({ error: 'العميل غير موجود' });

  try {
    // 1. Get all invoice IDs for this customer (needed to delete partial_payments)
    const { data: invoices } = await supabase.from('invoices').select('id').eq('customer_id', id);
    const invoiceIds = (invoices || []).map((inv: any) => inv.id);

    // 2. Delete partial_payments linked to customer's invoices
    if (invoiceIds.length > 0) {
      for (const invId of invoiceIds) {
        await supabase.from('partial_payments').delete().eq('invoice_id', invId);
      }
    }

    // 2.5 Delete preliminary bookings associated with this customer
    await supabase.from('preliminary_bookings').delete().eq('customer_name', customer.name);


    // 3. Get all booking/maintenance/fault IDs to clean up worker_payments & worker_settlements
    const [bookingsRes, maintRes, faultsRes] = await Promise.all([
      supabase.from('installation_bookings').select('id').eq('customer_id', id),
      supabase.from('maintenance_requests').select('id').eq('customer_id', id),
      supabase.from('fault_requests').select('id').eq('customer_id', id),
    ]);

    const bookingIds = (bookingsRes.data || []).map((b: any) => b.id);
    const maintIds = (maintRes.data || []).map((m: any) => m.id);
    const faultIds = (faultsRes.data || []).map((f: any) => f.id);

    // 4. Delete worker_payments, worker_settlements, and task_assignments for each booking
    for (const bId of bookingIds) {
      await supabase.from('worker_payments').delete().eq('task_id', bId).eq('task_type', 'booking');
      await supabase.from('worker_settlements').delete().eq('taskId', bId).eq('taskType', 'booking');
      await supabase.from('task_assignments').delete().eq('taskId', bId).eq('taskType', 'booking');
    }
    // Delete worker_payments, worker_settlements, and task_assignments for each maintenance request
    for (const mId of maintIds) {
      await supabase.from('worker_payments').delete().eq('task_id', mId).eq('task_type', 'maintenance');
      await supabase.from('worker_settlements').delete().eq('taskId', mId).eq('taskType', 'maintenance');
      await supabase.from('task_assignments').delete().eq('taskId', mId).eq('taskType', 'maintenance');
    }
    // Delete worker_payments, worker_settlements, and task_assignments for each fault request
    for (const fId of faultIds) {
      await supabase.from('worker_payments').delete().eq('task_id', fId).eq('task_type', 'fault');
      await supabase.from('worker_settlements').delete().eq('taskId', fId).eq('taskType', 'fault');
      await supabase.from('task_assignments').delete().eq('taskId', fId).eq('taskType', 'fault');
    }

    // 5. Delete all related records in parallel
    const deleteResults = await Promise.allSettled([
      supabase.from('invoices').delete().eq('customer_id', id),
      supabase.from('installment_records').delete().eq('customer_id', id),
      supabase.from('maintenance_requests').delete().eq('customer_id', id),
      supabase.from('fault_requests').delete().eq('customer_id', id),
      supabase.from('installation_bookings').delete().eq('customer_id', id),
      supabase.from('archive_records').delete().eq('customer_id', id),
      supabase.from('contracts').delete().eq('customer_id', id),
    ]);

    // Log any individual deletion errors but don't fail the whole operation
    deleteResults.forEach((result, idx) => {
      if (result.status === 'rejected') {
        console.error(`[CASCADE DELETE] Step ${idx} failed for customer ${id}:`, result.reason);
      } else if (result.value && (result.value as any).error) {
        console.error(`[CASCADE DELETE] Step ${idx} DB error for customer ${id}:`, (result.value as any).error);
      }
    });

    // 6. Finally delete the customer record itself (hard delete)
    const { error } = await supabase.from('customers').delete().eq('id', id);
    if (error) return res.status(500).json({ error: error.message });

    await logAudit(req.user.name, 'حذف عميل نهائي (مع جميع السجلات)', `${customer.name} / ID: ${id}`, getClientIp(req));
    res.json({ success: true });
  } catch (err: any) {
    console.error('[CASCADE DELETE ERROR]', err);
    return res.status(500).json({ error: 'فشل حذف العميل وسجلاته: ' + (err.message || 'خطأ غير متوقع') });
  }
});
...
app.get('/api/customers/:id/profile', authenticate, async (req: any, res) => {
  ...
  const id = parseInt(req.params.id);
  const [custRes, invRes, instRes, maintRes, faultRes, bookRes, archRes] = await Promise.all([
    ...
    supabase.from('archive_records').select('*').eq('customer_id', id)
  ]);
  ...
  res.json({
    ...
    archiveRecords: (archRes.data || []).map(mapArchive)
  });
});
...
app.post('/api/pos/invoice', authenticate, async (req: any, res) => {
  try {
    ...
    const { data: newInvoice, error: invErr } = await supabase.from('invoices').insert({
      ...
      pos_cash_advance: invoiceType === 'mastercard' ? paidVal : 0,
...
app.get('/api/bookings/:id/invoice', authenticate, async (req: any, res) => {
  ...
  const canViewBookingInvoice = perms?.installationBookings?.viewInvoice || perms?.installationBookings?.view || perms?.invoices?.view || perms?.sales?.view;
...
app.post('/api/maintenance', authenticate, async (req: any, res) => {
  ...
  const { customerId, archiveId, notes, photos, assignedEmployee } = req.body;
  if (!customerId && !archiveId) return res.status(400).json({ error: 'معرف العميل أو الأرشيف إلزامي' });
  if (!notes) return res.status(400).json({ error: 'الملاحظات إلزامية' });
  ...
  if (archiveId) {
    const { data: arch } = await supabase.from('archive_records').select('*').eq('id', parseInt(archiveId)).single();
    if (!arch) return res.status(404).json({ error: 'سجل الأرشيف غير موجود' });
    custName = arch.customer_name + ' (أرشيف)';
    custPhone = arch.customer_phone || '';
    custAddress = arch.installation_location || '';
  } else if (customerId) {
    const { data: customer } = await supabase.from('customers').select('*').eq('id', parseInt(customerId)).eq('is_deleted', false).single();
    if (!customer) return res.status(404).json({ error: 'العميل غير موجود' });
    custId = customer.id;
  ...
  const { data, error } = await supabase.from('maintenance_requests').insert({
    ...
    customer_id: custId, archive_id: archiveId ? parseInt(archiveId) : null,
  ...
  await logAudit(req.user.name, 'فتح طلب صيانة', `${requestNumber} - ${custName}`, getClientIp(req));
...
app.post('/api/faults', authenticate, async (req: any, res) => {
  ...
  const { customerId, archiveId, faultType, description, photos, notes } = req.body;
  if (!customerId && !archiveId) return res.status(400).json({ error: 'معرف العميل أو الأرشيف إلزامي' });
  if (!faultType || !description) return res.status(400).json({ error: 'بيانات إلزامية مفقودة' });
  ...
  await logAudit(req.user.name, 'تسجيل بلاغ عطل', `${faultType} - ${custName}`, getClientIp(req));
...
app.post('/api/installation-teams', authenticate, async (req: any, res) => {
  ...
  const { name, leader, members, vehicle, vehicleNumber, vehicleType, vehicleNotes, equipment } = req.body;
...
app.put('/api/installation-teams/:id', authenticate, async (req: any, res) => {
  ...
  if (vehicleNumber !== undefined) updates.vehicle_number = (vehicleNumber || '').trim();
  if (vehicleType !== undefined) updates.vehicle_type = (vehicleType || '').trim();
  if (vehicleNotes !== undefined) updates.vehicle_notes = (vehicleNotes || '').trim();
  if (equipment !== undefined) updates.equipment = Array.isArray(equipment) ? equipment : [];
...
const EXPENSE_CARDS_FILE = 'team_expense_cards.json';

const loadLocalExpenseCards = (): any[] => {
  try {
    if (fs.existsSync(EXPENSE_CARDS_FILE)) {
      const data = fs.readFileSync(EXPENSE_CARDS_FILE, 'utf-8');
      return JSON.parse(data || '[]');
    }
  } catch (e) {
    console.error('Failed to read team_expense_cards.json:', e);
  }
  return [];
};

const saveLocalExpenseCards = (cards: any[]) => {
  try {
    fs.writeFileSync(EXPENSE_CARDS_FILE, JSON.stringify(cards, null, 2), 'utf-8');
  } catch (e) {
    console.error('Failed to write team_expense_cards.json:', e);
  }
};

app.get('/api/installation-teams/:teamId/expense-cards', authenticate, async (req: any, res) => {
  const teamId = parseInt(req.params.teamId);
  if (!teamId) return res.status(400).json({ error: 'رقم الطاقم غير صحيح' });

  try {
    const { data: cards, error: cardsErr } = await supabase
      .from('team_expense_cards')
      .select('*')
      .eq('team_id', teamId)
      .order('id', { ascending: false });

    if (cardsErr) {
      // Table pending creation in Supabase -> fallback to local store
      const localCards = loadLocalExpenseCards().filter(c => c.teamId === teamId);
      return res.json(localCards);
    }
    ...
    const mappedCards = (cards || []).map((card: any) => {
      const cardItems = items
        .filter((it: any) => it.card_id === card.id)
        .map((it: any) => ({
          id: it.id,
          cardId: it.card_id,
          description: it.description,
          amount: parseFloat(it.amount) || 0,
          itemDate: it.item_date || it.created_at?.split('T')[0],
          createdBy: it.created_by,
          createdAt: it.created_at
        }));

      const totalAmount = parseFloat(card.total_amount) || 0;
      const spentAmount = cardItems.reduce((sum: number, it: any) => sum + it.amount, 0);
      ...
      return {
        ...
        title: card.title,
        totalAmount,
        cardDate: card.card_date || card.created_at?.split('T')[0],
        notes: card.notes || '',
        spentAmount,
        remainingAmount,
        items: cardItems,
    ...
    // Merge with any local cards if table empty
    if (mappedCards.length === 0) {
      const localCards = loadLocalExpenseCards().filter(c => c.teamId === teamId);
      if (localCards.length > 0) return res.json(localCards);
    }
...
app.post('/api/installation-teams/:teamId/expense-cards', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationTeams?.edit && !req.user.permissions.installationTeams?.create) {
    return res.status(403).json({ error: 'ليس لديك صلاحية لإضافة كروت الصرفيات' });
  }

  const teamId = parseInt(req.params.teamId);
  const { title, totalAmount, cardDate, notes } = req.body;

  if (!teamId || !title || totalAmount === undefined || parseFloat(totalAmount) <= 0) {
    return res.status(400).json({ error: 'عنوان الكارت والمبلغ المضاف إلزاميان' });
  }
  ...
  try {
    const { data: newCard, error } = await supabase
      ...
      .insert({
        ...
        card_date: cardDate || new Date().toISOString().split('T')[0],
        notes: (notes || '').trim(),
        created_by: req.user.name
      })
      .select()
    ...
    if (error) {
      // Supabase table not created yet -> Fallback to local store
      const localCards = loadLocalExpenseCards();
      const localNewCard = {
        id: Date.now(),
        teamId,
        title: title.trim(),
        totalAmount: parseFloat(totalAmount) || 0,
        cardDate: cardDate || new Date().toISOString().split('T')[0],
        notes: (notes || '').trim(),
        spentAmount: 0,
        remainingAmount: parseFloat(totalAmount) || 0,
        items: [],
        createdBy: req.user.name,
        createdAt: new Date().toISOString()
      };
      localCards.unshift(localNewCard);
      saveLocalExpenseCards(localCards);

      await logAudit(req.user.name, 'إنشاء كارت ميزانية طاقم', `طاقم: ${teamId}, الكارت: ${title}, المبلغ: ${totalAmount}`, getClientIp(req));
      return res.status(201).json(localNewCard);
    }
    ...
    res.status(201).json({
      id: newCard.id,
      teamId: newCard.team_id,
      title: newCard.title,
      totalAmount: parseFloat(newCard.total_amount) || 0,
      cardDate: newCard.card_date,
      notes: newCard.notes,
      spentAmount: 0,
      remainingAmount: parseFloat(newCard.total_amount) || 0,
      items: [],
    ...
    localCards.unshift(localNewCard);
    saveLocalExpenseCards(localCards);
    res.status(201).json(localNewCard);
  }
});

app.delete('/api/installation-teams/expense-cards/:cardId', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationTeams?.delete && !req.user.permissions.installationTeams?.edit) {
    return res.status(403).json({ error: 'ليس لديك صلاحية لحذف كارت الصرفيات' });
  }

  const cardId = parseInt(req.params.cardId);
  if (!cardId) return res.status(400).json({ error: 'معرف الكارت غير صحيح' });

  try {
    await supabase.from('team_expense_cards').delete().eq('id', cardId);
    
    // Also delete from local store
    const localCards = loadLocalExpenseCards().filter(c => c.id !== cardId);
    saveLocalExpenseCards(localCards);

    await logAudit(req.user.name, 'حذف كارت ميزانية طاقم', `رقم: ${cardId}`, getClientIp(req));
    res.json({ success: true });
  } catch (err: any) {
    const localCards = loadLocalExpenseCards().filter(c => c.id !== cardId);
    saveLocalExpenseCards(localCards);
...
app.post('/api/installation-teams/expense-cards/:cardId/items', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationTeams?.edit && !req.user.permissions.installationTeams?.create) {
    return res.status(403).json({ error: 'ليس لديك صلاحية لإضافة مصروفات للكارت' });
  }

  const cardId = parseInt(req.params.cardId);
  const { description, amount, itemDate } = req.body;

  if (!cardId || !description || amount === undefined || parseFloat(amount) <= 0) {
    return res.status(400).json({ error: 'بيان الغرض والمبلغ المصروف إلزامي وجودهما' });
  }
  ...
  try {
    const { data: newItem, error } = await supabase
      ...
      .insert({
        ...
        item_date: itemDate || new Date().toISOString().split('T')[0],
        created_by: req.user.name
      })
      .select()
      .single();
    ...
    if (error) {
      ...
      const localCards = loadLocalExpenseCards();
      const cardIndex = localCards.findIndex(c => c.id === cardId);
      if (cardIndex >= 0) {
        const localNewItem = {
          id: Date.now(),
          cardId,
        ...
        const items = localCards[cardIndex].items || [];
        items.push(localNewItem);
        localCards[cardIndex].items = items;
        const spentAmount = items.reduce((sum: number, it: any) => sum + (it.amount || 0), 0);
        localCards[cardIndex].spentAmount = spentAmount;
        localCards[cardIndex].remainingAmount = localCards[cardIndex].totalAmount - spentAmount;
        saveLocalExpenseCards(localCards);

        await logAudit(req.user.name, 'تسجيل مصروف كارت طاقم', `كارت: ${cardId}, الغرض: ${description}, المبلغ: ${amount}`, getClientIp(req));
        return res.status(201).json(localNewItem);
      }
      return res.status(404).json({ error: 'الكارت غير موجود' });
    }
    ...
    res.status(201).json({
      id: newItem.id,
      cardId: newItem.card_id,
      description: newItem.description,
      amount: parseFloat(newItem.amount) || 0,
      itemDate: newItem.item_date,
...
app.delete('/api/installation-teams/expense-items/:itemId', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationTeams?.delete && !req.user.permissions.installationTeams?.edit) {
    return res.status(403).json({ error: 'ليس لديك صلاحية لحذف غرض الصرف' });
  }

  const itemId = parseInt(req.params.itemId);
  if (!itemId) return res.status(400).json({ error: 'معرف الغرض غير صحيح' });

  try {
    await supabase.from('team_expense_items').delete().eq('id', itemId);
    
    // Also delete from local store
    const localCards = loadLocalExpenseCards();
    localCards.forEach(card => {
      if (card.items) {
        card.items = card.items.filter((it: any) => it.id !== itemId);
        const spentAmount = card.items.reduce((sum: number, it: any) => sum + (it.amount || 0), 0);
        card.spentAmount = spentAmount;
        card.remainingAmount = card.totalAmount - spentAmount;
      }
    });
    saveLocalExpenseCards(localCards);

    await logAudit(req.user.name, 'حذف مصروف كارت طاقم', `رقم: ${itemId}`, getClientIp(req));
    res.json({ success: true });
  } catch (err: any) {
    const localCards = loadLocalExpenseCards();
    localCards.forEach(card => {
...
app.get('/api/preliminary-bookings', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.view) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { data, error } = await supabase.from('preliminary_bookings').select('*').order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.post('/api/preliminary-bookings', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.create) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const payload = {
    customer_name: req.body.customerName || '',
    phone_number: req.body.phoneNumber || '',
    total_amount: req.body.totalAmount || '',
    system_size: req.body.systemSize || '',
    panel_details: req.body.panelDetails || '',
    battery_details: req.body.batteryDetails || '',
    inverter_details: req.body.inverterDetails || '',
    status: req.body.status || 'pending',
    has_advance_payment: req.body.hasAdvancePayment ? true : false,
    advance_amount: req.body.advanceAmount || null,
    payment_method: req.body.paymentMethod || null,
    advance_status: req.body.advanceStatus || null
  };
  const { data, error } = await supabase.from('preliminary_bookings').insert(payload).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.status(201).json(data);
});

app.delete('/api/preliminary-bookings/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.delete) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { error } = await supabase.from('preliminary_bookings').delete().eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});

app.put('/api/preliminary-bookings/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.edit) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const payload = {
    customer_name: req.body.customerName || '',
    phone_number: req.body.phoneNumber || '',
    total_amount: req.body.totalAmount || '',
  ...
  const { data, error } = await supabase.from('preliminary_bookings').update(payload).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.put('/api/preliminary-bookings/:id/status', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.edit) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { status } = req.body;
  const { data, error } = await supabase.from('preliminary_bookings').update({ status }).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.put('/api/preliminary-bookings/:id/advance-status', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.edit) return res.status(403).json({ error: 'لا تملك صلاحية' });
  const { advanceStatus } = req.body;
  const { data, error } = await supabase.from('preliminary_bookings').update({ advance_status: advanceStatus }).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.post('/api/installations', authenticate, async (req: any, res) => {
  ...
  const custRes = await supabase.from('customers').select('*').eq('id', parseInt(customerId)).eq('is_deleted', false).single();
  if (!custRes.data) {
  ...
  let invData = null;
  if (invoiceId) {
    const invRes = await supabase.from('invoices').select('*').eq('id', parseInt(invoiceId)).single();
    if (!invRes.data) return res.status(404).json({ error: 'الفاتورة غير موجودة' });
    invData = invRes.data;
  }
  ...
  const { data, error } = await supabase.from('installation_bookings').insert({
    ...
    invoice_id: invData ? invData.id : null, 
    invoice_number: invData ? invData.invoice_number : 'مبدئي',
...
app.put('/api/installations/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.installationBookings.edit && !req.user.permissions.installationBookings.view) return res.status(403).json({ error: 'غير مصرح' });

  const { appointmentDate, appointmentTime, assignedTeamId, status, notes, invoiceId } = req.body;
  const updates: any = {};
  ...
  if (notes !== undefined) updates.notes = notes;
  if (invoiceId) {
    const { data: inv } = await supabase.from('invoices').select('*').eq('id', parseInt(invoiceId)).single();
    if (inv) {
      updates.invoice_id = inv.id;
      updates.invoice_number = inv.invoice_number;
    }
  }
...
app.get('/api/audits', authenticate, async (req: any, res) => {
  ...
  // Use created_at for ordering instead of id — the SERIAL sequence may have gaps after DB migration
...
app.get('/api/archive', authenticate, async (req: any, res) => {
  if (!req.user.permissions.archive?.view && !req.user.permissions.maintenance?.view && !req.user.permissions.faults?.view) {
    return res.status(403).json({ error: 'لا تملك صلاحية' });
  }
  const { data, error } = await supabase.from('archive_records').select('*').order('id', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json((data || []).map(mapArchive));
});

app.post('/api/archive', authenticate, async (req: any, res) => {
  if (!req.user.permissions.archive?.create) return res.status(403).json({ error: 'لا تملك صلاحية' });

  const { customerName, installationDate, systemSize, customerPhone, inverterSize, batteriesCount, panelsCount, installationLocation, notes, price } = req.body;
  if (!customerName) return res.status(400).json({ error: 'اسم الزبون مطلوب' });
  ...
  if (customerPhone || customerName) {
    ...
    if (customerPhone) {
      ...
      q = q.eq('name', customerName);
    }
    const { data: existingCust } = await q.maybeSingle();
    ...
    if (existingCust) {
      ...
      const { data: newCust } = await supabase.from('customers').insert({
        ...
        address: installationLocation || 'العنوان غير محدد'
      }).select('id').single();
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'إضافة للأرشيف', customerName, getClientIp(req));
  res.status(201).json(mapArchive(data));
});

app.put('/api/archive/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.archive?.edit) return res.status(403).json({ error: 'لا تملك صلاحية' });
  ...
  if (systemSize !== undefined) updates.system_size = systemSize;
  if (customerPhone !== undefined) updates.customer_phone = customerPhone;
  if (inverterSize !== undefined) updates.inverter_size = inverterSize;
  if (batteriesCount !== undefined) updates.batteries_count = batteriesCount;
  if (panelsCount !== undefined) updates.panels_count = panelsCount;
  if (installationLocation !== undefined) updates.installation_location = installationLocation;
  if (notes !== undefined) updates.notes = notes;
  if (price !== undefined) updates.price = price !== '' ? parseFloat(price) || 0 : 0;

  const { data, error } = await supabase.from('archive_records').update(updates).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });

  await logAudit(req.user.name, 'تعديل في الأرشيف', `ID: ${req.params.id}`, getClientIp(req));
  res.json(mapArchive(data));
});

app.delete('/api/archive/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.archive?.delete) return res.status(403).json({ error: 'لا تملك صلاحية' });

  const { error } = await supabase.from('archive_records').delete().eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });

  await logAudit(req.user.name, 'حذف من الأرشيف', `ID: ${req.params.id}`, getClientIp(req));
  res.json({ success: true });
});

// ==================== FIX SEQUENCE (Admin) ====================
// After restoring a backup, PostgreSQL SERIAL sequences get out of sync.
// This endpoint inserts a sentinel row to advance the sequence past the max ID.
app.post('/api/admin/fix-sequences', authenticate, async (req: any, res) => {
  if (!req.user.permissions.settings?.edit && !req.user.permissions.backups?.approve) {
    return res.status(403).json({ error: 'ليس لديك صلاحية' });
  }

  const tables: { table: string; idCol: string }[] = [
    { table: 'audit_logs', idCol: 'id' },
    { table: 'customers', idCol: 'id' },
    { table: 'products', idCol: 'id' },
    { table: 'invoices', idCol: 'id' },
    { table: 'installment_records', idCol: 'id' },
    { table: 'installation_teams', idCol: 'id' },
    { table: 'installation_bookings', idCol: 'id' },
    { table: 'maintenance_requests', idCol: 'id' },
    { table: 'fault_requests', idCol: 'id' },
    { table: 'partial_payments', idCol: 'id' },
    { table: 'bank_withdrawals', idCol: 'id' },
    { table: 'contracts', idCol: 'id' },
    { table: 'roles', idCol: 'id' },
    { table: 'installation_workers', idCol: 'id' },
    { table: 'worker_settlements', idCol: 'id' },
    { table: 'task_assignments', idCol: 'id' },
    { table: 'worker_payments', idCol: 'id' },
    { table: 'inventory_movements', idCol: 'id' },
  ];
  ...
  for (const { table } of tables) {
    try {
      ...
      // Check what the next sequence value would be by inserting and checking
      // We do a minimal insert to probe the sequence, then delete it
      // For most tables we can't easily do this without knowing required fields.
      // Instead: if maxId > 0, we upsert a record with that exact ID to make sure
      // the sequence is at least at maxId. But upsert doesn't advance SERIAL.
      // The real fix needs setval() which requires direct DB access.
      // What we CAN do: mark the result for the admin to see.
      results.push({ table, maxId, note: 'check required if sequence < maxId' });
    } catch (e: any) {
      results.push({ table, error: e.message });
    }
  }

  // The actual fix: audit_logs specifically - insert a record with explicit id = max+gap
  // and rely on SERIAL to skip past it. This is a workaround.
  try {
    const { data: maxAudit } = await supabase
      .from('audit_logs')
      .select('id')
      .order('id', { ascending: false })
      .limit(1);
    const maxAuditId = maxAudit?.[0]?.id || 0;

    // Insert a probe record WITHOUT specifying id, to find what sequence gives us
    const now = new Date();
    const { data: probe, error: probeErr } = await supabase.from('audit_logs').insert({
      user: 'SYSTEM',
      date: now.toISOString().split('T')[0],
      time: now.toTimeString().split(' ')[0],
      action: 'إصلاح تسلسل قاعدة البيانات',
      affected_record: `تم استدعاء fix-sequences، أعلى ID: ${maxAuditId}`,
      ip_address: getClientIp(req)
    }).select().single();

    if (probe) {
      const seqId = probe.id;
      const isBroken = seqId < maxAuditId;
      results.push({
        table: 'audit_logs',
        maxId: maxAuditId,
        sequenceGave: seqId,
        broken: isBroken,
        message: isBroken
          ? `⚠️ SEQUENCE منكسر! يعطي ID ${seqId} بينما الحد الأعلى ${maxAuditId}. يرجى تشغيل: SELECT setval('audit_logs_id_seq', ${maxAuditId}, true); في Supabase SQL Editor`
          : `✅ SEQUENCE سليم`
      });
      // Don't delete - keep as a real audit entry
    }
  } catch(e: any) {
    results.push({ table: 'audit_logs_probe', error: e.message });
  }
...
app.get('/api/backup/download', authenticate, async (req: any, res) => {
  ...
  const [
    ...
    contracts, roles, installationWorkers, workerSettlements, taskAssignments, workerPayments
  ] = await Promise.all([
    ...
    supabase.from('customers').select('*').limit(100000),
    supabase.from('invoices').select('*').limit(100000),
    supabase.from('installment_records').select('*').limit(100000),
    supabase.from('installation_teams').select('*').limit(100000),
    supabase.from('installation_bookings').select('*').limit(100000),
    supabase.from('maintenance_requests').select('*').limit(100000),
    supabase.from('fault_requests').select('*').limit(100000),
    supabase.from('settings').select('*').limit(100000),
    supabase.from('audit_logs').select('*').order('created_at', { ascending: false }).limit(100000),
    supabase.from('inventory_movements').select('*').order('id', { ascending: false }).limit(100000),
    supabase.from('partial_payments').select('*').limit(100000),
    supabase.from('bank_withdrawals').select('*').limit(100000),
    supabase.from('contracts').select('*').limit(100000),
    supabase.from('roles').select('*').limit(100000),
    supabase.from('installation_workers').select('*').limit(100000),
    supabase.from('worker_settlements').select('*').limit(100000),
    supabase.from('task_assignments').select('*').limit(100000),
    supabase.from('worker_payments').select('*').limit(100000)
  ]);
  ...
  res.json({
    ...
    workerPayments: workerPayments.data || []
  });
});
...
app.post('/api/backup/restore', authenticate, async (req: any, res) => {
  ...
  try {
    ...
    if (backup.workerPayments && backup.workerPayments.length > 0) {
      await supabase.from('worker_payments').upsert(backup.workerPayments, { onConflict: 'id' });
    }
    if (backup.auditLogs && backup.auditLogs.length > 0) {
      await supabase.from('audit_logs').upsert(backup.auditLogs, { onConflict: 'id' });
    }
    if (backup.inventoryMovements && backup.inventoryMovements.length > 0) {
      await supabase.from('inventory_movements').upsert(backup.inventoryMovements, { onConflict: 'id' });
    }
    if (backup.settings && Object.keys(backup.settings).length > 0) {
      await supabase.from('settings').upsert([backup.settings], { onConflict: 'id' });
    }
    
    if (backup.users && backup.users.length > 0) {
      await supabase.from('users').upsert(backup.users, { onConflict: 'id' });
    }
...
function mapExchangeOffice(r: any) {
  return {
    id: r.id, name: r.name, phone: r.phone || '', city: r.city || '',
    address: r.address || '', notes: r.notes || '',
    initialBalanceIqd: parseFloat(r.initial_balance_iqd) || 0,
    initialBalanceUsd: parseFloat(r.initial_balance_usd) || 0,
    status: r.status || 'active', createdBy: r.created_by, createdAt: r.created_at
  };
}

function mapExchangeTransaction(r: any) {
  return {
    id: r.id, officeId: r.office_id, voucherNumber: r.voucher_number,
    transactionType: r.transaction_type, description: r.description || '',
    amount: parseFloat(r.amount) || 0, currency: r.currency || 'IQD',
    direction: r.direction, runningBalance: parseFloat(r.running_balance) || 0,
    reference: r.reference || '', sourceModule: r.source_module,
    sourceId: r.source_id, attachments: r.attachments || [],
    notes: r.notes || '', transactionDate: r.transaction_date,
    createdBy: r.created_by, createdAt: r.created_at
  };
}

// List all exchange offices
app.get('/api/exchange-offices', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });

  const { data, error } = await supabase.from('exchange_offices')
    .select('*').order('id', { ascending: false });

  if (error) return res.status(500).json({ error: error.message });
  res.json((data || []).map(mapExchangeOffice));
});

// Create exchange office
app.post('/api/exchange-offices', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });

  const { name, phone, city, address, notes, initialBalanceIqd, initialBalanceUsd, status } = req.body;
  if (!name?.trim()) return res.status(400).json({ error: 'اسم الصيرفة مطلوب' });

  const { data, error } = await supabase.from('exchange_offices').insert({
    name: name.trim(), phone: phone || '', city: city || '', address: address || '',
    notes: notes || '', initial_balance_iqd: parseFloat(initialBalanceIqd) || 0,
    initial_balance_usd: parseFloat(initialBalanceUsd) || 0,
    status: status || 'active', created_by: req.user.name
  }).select().single();

  if (error) return res.status(500).json({ error: error.message });

  // Create opening balance transactions if initial balances are set
  const iqdBal = parseFloat(initialBalanceIqd) || 0;
  const usdBal = parseFloat(initialBalanceUsd) || 0;

  if (iqdBal !== 0) {
    await supabase.from('exchange_transactions').insert({
      office_id: data.id, voucher_number: `EX-${data.id}-0001`,
      transaction_type: 'opening_balance', description: 'رصيد افتتاحي',
      amount: Math.abs(iqdBal), currency: 'IQD',
      direction: iqdBal >= 0 ? 'credit' : 'debit',
      running_balance: iqdBal, reference: '',
  ...
  if (usdBal !== 0) {
    await supabase.from('exchange_transactions').insert({
      office_id: data.id, voucher_number: `EX-${data.id}-0002`,
      transaction_type: 'opening_balance', description: 'رصيد افتتاحي',
      amount: Math.abs(usdBal), currency: 'USD',
      direction: usdBal >= 0 ? 'credit' : 'debit',
      running_balance: usdBal, reference: '',
      transaction_date: new Date().toISOString().split('T')[0],
      created_by: req.user.name
    });
  }

  await logAudit(req.user.name, 'إنشاء صيرفة', name, getClientIp(req));
  res.status(201).json(mapExchangeOffice(data));
});

// Update exchange office
app.put('/api/exchange-offices/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.edit) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const { name, phone, city, address, notes, status } = req.body;
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'تعديل صيرفة', `ID: ${id}`, getClientIp(req));
  res.json(mapExchangeOffice(data));
});

// Delete exchange office
app.delete('/api/exchange-offices/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);

  const { data: office } = await supabase.from('exchange_offices').select('name').eq('id', id).single();
  if (!office) return res.status(404).json({ error: 'الصيرفة غير موجودة' });

  // Cascade will delete transactions
  const { error } = await supabase.from('exchange_offices').delete().eq('id', id);
  if (error) return res.status(500).json({ error: error.message });

  await logAudit(req.user.name, 'حذف صيرفة', office.name, getClientIp(req));
  res.json({ success: true });
});

// Dashboard summary — must be before /:id routes
app.get('/api/exchange-offices/dashboard', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });

  const { data: offices } = await supabase.from('exchange_offices').select('id, name, status');
  const activeOffices = (offices || []).filter((o: any) => o.status === 'active');
  const totalOffices = activeOffices.length;

  // Get latest balances for each office (last transaction per currency)
  let totalBalanceIqd = 0;
  let totalBalanceUsd = 0;
  let highestOffice = { name: '-', balance: 0 };
  let lowestOffice = { name: '-', balance: Infinity };

  for (const office of activeOffices) {
    // Get last IQD balance
    const { data: lastIqd } = await supabase.from('exchange_transactions')
      .select('running_balance')
      .eq('office_id', office.id).eq('currency', 'IQD')
      .order('id', { ascending: false }).limit(1);
    const iqd = lastIqd?.[0] ? parseFloat(lastIqd[0].running_balance) || 0 : 0;
    totalBalanceIqd += iqd;

    // Get last USD balance
    const { data: lastUsd } = await supabase.from('exchange_transactions')
      .select('running_balance')
      .eq('office_id', office.id).eq('currency', 'USD')
      .order('id', { ascending: false }).limit(1);
    const usd = lastUsd?.[0] ? parseFloat(lastUsd[0].running_balance) || 0 : 0;
    totalBalanceUsd += usd;

    const combined = iqd + (usd * 1480); // Approximate USD→IQD for comparison
    if (combined > highestOffice.balance) highestOffice = { name: office.name, balance: combined };
    if (combined < lowestOffice.balance) lowestOffice = { name: office.name, balance: combined };
  }

  if (lowestOffice.balance === Infinity) lowestOffice = { name: '-', balance: 0 };

  // Today's transactions
  const today = new Date().toISOString().split('T')[0];
  const { count: todayCount } = await supabase.from('exchange_transactions')
    .select('id', { count: 'exact', head: true })
    .eq('transaction_date', today);

  // This month transactions
  const monthStart = today.substring(0, 7) + '-01';
  const { count: monthCount } = await supabase.from('exchange_transactions')
    .select('id', { count: 'exact', head: true })
...
// Office summary / profile cards
app.get('/api/exchange-offices/:id/summary', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  ...
  const { data: lastIqd } = await supabase.from('exchange_transactions')
    .select('running_balance')
    .eq('office_id', id).eq('currency', 'IQD')
    .order('id', { ascending: false }).limit(1);

  // Get last USD balance
  const { data: lastUsd } = await supabase.from('exchange_transactions')
    .select('running_balance')
    .eq('office_id', id).eq('currency', 'USD')
    .order('id', { ascending: false }).limit(1);

  // Total received
  const { data: totalReceivedData } = await supabase.from('exchange_transactions')
    .select('amount, currency').eq('office_id', id).eq('direction', 'credit');
  const totalReceivedIqd = (totalReceivedData || []).filter((t: any) => t.currency === 'IQD').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);
  const totalReceivedUsd = (totalReceivedData || []).filter((t: any) => t.currency === 'USD').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);

  // Total paid
  const { data: totalPaidData } = await supabase.from('exchange_transactions')
    .select('amount, currency').eq('office_id', id).eq('direction', 'debit');
  const totalPaidIqd = (totalPaidData || []).filter((t: any) => t.currency === 'IQD').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);
  const totalPaidUsd = (totalPaidData || []).filter((t: any) => t.currency === 'USD').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);

  // Total transfers
  const { count: totalTransfers } = await supabase.from('exchange_transactions')
    .select('id', { count: 'exact', head: true })
    .eq('office_id', id);

  // Last transaction
  const { data: lastTx } = await supabase.from('exchange_transactions')
    .select('*').eq('office_id', id)
    .order('id', { ascending: false }).limit(1);

  res.json({
    balanceIqd: lastIqd?.[0] ? parseFloat(lastIqd[0].running_balance) || 0 : 0,
    balanceUsd: lastUsd?.[0] ? parseFloat(lastUsd[0].running_balance) || 0 : 0,
    totalReceivedIqd,
    totalReceivedUsd,
    totalPaidIqd,
    totalPaidUsd,
    totalTransfers: totalTransfers || 0,
    lastTransaction: lastTx?.[0] ? mapExchangeTransaction(lastTx[0]) : null
  });
});

// List transactions for an office — PAGINATED with SERVER-SIDE FILTERS
app.get('/api/exchange-offices/:id/transactions', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const page = parseInt(req.query.page as string) || 1;
  ...
  const voucherNumber = (req.query.voucherNumber as string || '').trim();
  const search = (req.query.search as string || '').trim();

  let query = supabase.from('exchange_transactions')
    .select('*', { count: 'exact' })
    .eq('office_id', id);
  ...
  if (voucherNumber) query = query.ilike('voucher_number', `%${voucherNumber}%`);
  if (search) {
    const safeSearch = search.replace(/[,()\"\\%]/g, ' ').replace(/\s+/g, ' ').trim();
    if (safeSearch) query = query.or(`description.ilike.%${safeSearch}%,reference.ilike.%${safeSearch}%,notes.ilike.%${safeSearch}%,voucher_number.ilike.%${safeSearch}%`);
  }
  ...
  res.json({
    data: (data || []).map(mapExchangeTransaction),
    total: count || 0,
    page,
    limit
  });
...
// Create transaction — auto-computes running balance and voucher number
app.post('/api/exchange-offices/:id/transactions', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const officeId = parseInt(req.params.id);
  const { transactionType, description, amount, currency, direction, reference, notes, transactionDate, sourceModule, sourceId } = req.body;

  if (!transactionType || !amount || !direction) {
    return res.status(400).json({ error: 'نوع المعاملة والمبلغ والاتجاه مطلوبة' });
  }

  const amt = parseFloat(amount);
  if (isNaN(amt) || amt <= 0) return res.status(400).json({ error: 'المبلغ يجب أن يكون أكبر من صفر' });

  const curr = currency || 'IQD';

  // Get last running balance for this office + currency
  const { data: lastTx } = await supabase.from('exchange_transactions')
    .select('running_balance')
    .eq('office_id', officeId).eq('currency', curr)
    .order('id', { ascending: false }).limit(1);

  const prevBalance = lastTx?.[0] ? parseFloat(lastTx[0].running_balance) || 0 : 0;
  const newBalance = direction === 'credit' ? prevBalance + amt : prevBalance - amt;

  // Generate voucher number
  const { count: txCount } = await supabase.from('exchange_transactions')
    .select('id', { count: 'exact', head: true })
    .eq('office_id', officeId);

  const seq = ((txCount || 0) + 1).toString().padStart(4, '0');
  const voucherNumber = `EX-${officeId}-${seq}`;

  const { data, error } = await supabase.from('exchange_transactions').insert({
    office_id: officeId, voucher_number: voucherNumber,
    transaction_type: transactionType, description: description || '',
    amount: amt, currency: curr, direction,
    running_balance: newBalance, reference: reference || '',
    source_module: sourceModule || null, source_id: sourceId || null,
    notes: notes || '',
  ...
  await logAudit(req.user.name, 'إنشاء حركة مالية', `صيرفة: ${officeId}, ${voucherNumber}, ${amt} ${curr}`, getClientIp(req));
  res.status(201).json(mapExchangeTransaction(data));
});

// Update transaction
app.put('/api/exchange-offices/transactions/:txId', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.edit) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const txId = parseInt(req.params.txId);
  const { description, reference, notes, transactionDate } = req.body;
  ...
  const { data, error } = await supabase.from('exchange_transactions')
    .update(updates).eq('id', txId).select().single();

  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'تعديل حركة مالية', `ID: ${txId}`, getClientIp(req));
  res.json(mapExchangeTransaction(data));
});

// Delete transaction and recalculate subsequent running balances
app.delete('/api/exchange-offices/transactions/:txId', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const txId = parseInt(req.params.txId);

  // Get the transaction to delete
  const { data: tx } = await supabase.from('exchange_transactions').select('*').eq('id', txId).single();
  if (!tx) return res.status(404).json({ error: 'الحركة غير موجودة' });

  const officeId = tx.office_id;
  const curr = tx.currency;

  // Delete the transaction
  const { error } = await supabase.from('exchange_transactions').delete().eq('id', txId);
  if (error) return res.status(500).json({ error: error.message });

  // Recalculate running balances for all subsequent transactions of same office+currency
  const { data: allTx } = await supabase.from('exchange_transactions')
    .select('id, amount, direction, running_balance')
    .eq('office_id', officeId).eq('currency', curr)
    .order('id', { ascending: true });
  ...
  if (allTx && allTx.length > 0) {
    ...
    for (const t of allTx) {
      ...
      runBal = t.direction === 'credit' ? runBal + amt : runBal - amt;
      if (Math.abs(runBal - (parseFloat(t.running_balance) || 0)) > 0.001) {
        await supabase.from('exchange_transactions')
          .update({ running_balance: runBal }).eq('id', t.id);
      }
    }
  }

  await logAudit(req.user.name, 'حذف حركة مالية', `صيرفة: ${officeId}, ID: ${txId}`, getClientIp(req));
  res.json({ success: true });
});

// Account statement / report
app.get('/api/exchange-offices/:id/report', authenticate, async (req: any, res) => {
  if (!req.user.permissions.moneyTransfers?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const dateFrom = req.query.dateFrom as string;
  const dateTo = req.query.dateTo as string;
  const currency = (req.query.currency as string) || 'IQD';

  // Get office details
  const { data: office } = await supabase.from('exchange_offices').select('*').eq('id', id).single();
  if (!office) return res.status(404).json({ error: 'الصيرفة غير موجودة' });

  // Opening balance: last transaction before dateFrom, or 0
  let openingBalance = 0;
  if (dateFrom) {
    const { data: prevTx } = await supabase.from('exchange_transactions')
      .select('running_balance')
      .eq('office_id', id).eq('currency', currency)
      .lt('transaction_date', dateFrom)
      .order('id', { ascending: false }).limit(1);
    openingBalance = prevTx?.[0] ? parseFloat(prevTx[0].running_balance) || 0 : 0;
  }
  ...
  const { data: transactions, error } = await query.order('id', { ascending: true });
  if (error) return res.status(500).json({ error: error.message });

  const txList = (transactions || []).map(mapExchangeTransaction);

  const totalReceived = txList.filter(t => t.direction === 'credit').reduce((s, t) => s + t.amount, 0);
  const totalPaid = txList.filter(t => t.direction === 'debit').reduce((s, t) => s + t.amount, 0);
  const closingBalance = txList.length > 0 ? txList[txList.length - 1].runningBalance : openingBalance;

  res.json({
    office: mapExchangeOffice(office),
    currency,
...
function mapInvoice(r: any, customerAddress?: string | number) {
  return {
    ...
    posCashAdvance: r.pos_cash_advance || 0,
...
function mapFault(r: any) {
  const rawDate = r.created_date || r.created_at || '';
  const formattedDate = rawDate && rawDate.length > 10 ? rawDate.split('T')[0] : rawDate;
  return {
    id: r.id, customerId: r.customer_id, customerName: r.customer_name,
    customerPhone: r.customer_phone, customerAddress: r.customer_address,
    faultType: r.fault_type, description: r.description,
    photos: r.photos, notes: r.notes, status: r.status, createdDate: formattedDate,
    archiveId: r.archive_id
  };
}
...
function mapArchive(r: any) {
  return {
    id: r.id, customerId: r.customer_id, customerName: r.customer_name, installationDate: r.installation_date,
    systemSize: r.system_size, customerPhone: r.customer_phone, inverterSize: r.inverter_size,
    batteriesCount: r.batteries_count, panelsCount: r.panels_count, installationLocation: r.installation_location,
    notes: r.notes, price: r.price !== null && r.price !== undefined ? parseFloat(r.price) || 0 : undefined,
...
function getDefaultRoles() {
  const allTrue = { view: true, create: true, edit: true, delete: true, approve: true, export: true, showProfitMargin: true };
...
function mapContract(c: any) {
  return {
    ...
    manualCustomerName: c.manual_customer_name || '',
    manualCustomerPhone: c.manual_customer_phone || '',
    manualCustomerAddress: c.manual_customer_address || '',
    manualInvoiceNumber: c.manual_invoice_number || ''
  };
}
...
app.post('/api/contracts', authenticate, async (req: any, res) => {
  ...
  const {
    ...
    isLegacy, manualCustomerName, manualCustomerPhone, manualCustomerAddress, manualInvoiceNumber
  } = req.body;

  // Require either a linked customer OR a manual name for legacy contracts
  if (!isLegacy && !customerId) return res.status(400).json({ error: 'يرجى تحديد العميل' });
  if (isLegacy && !manualCustomerName?.trim()) return res.status(400).json({ error: 'يرجى إدخال اسم العميل' });
  ...
  let nextSequence = 1;
  if (latestContract && latestContract.contract_number) {
  ...
  const contractNumber = `CON-${year}-${nextSequence.toString().padStart(4, '0')}`;

  const { data, error } = await supabase.from('contracts').insert({
    contract_number: contractNumber,
    customer_id: isLegacy ? null : (customerId || null),
    invoice_id: isLegacy ? null : (invoiceId || null),
    ...
    manual_customer_name: isLegacy ? (manualCustomerName || '') : '',
    manual_customer_phone: isLegacy ? (manualCustomerPhone || '') : '',
    manual_customer_address: isLegacy ? (manualCustomerAddress || '') : '',
    manual_invoice_number: isLegacy ? (manualInvoiceNumber || '') : '',
    created_at: req.body.contractDate ? new Date(req.body.contractDate).toISOString() : new Date().toISOString()
  }).select().single();

  if (error) return res.status(500).json({ error: error.message });

  await logAudit(req.user.name, isLegacy ? 'إنشاء عقد قديم يدوي' : 'إنشاء عقد جديد', `رقم العقد: ${contractNumber}`, getClientIp(req));
...
app.put('/api/contracts/:id', authenticate, async (req: any, res) => {
  ...
  if (req.body.contractDate) {
    updates.created_at = new Date(req.body.contractDate).toISOString();
  }
  ...
  // Handle legacy manual fields
  if (req.body.isLegacy !== undefined) updates.is_legacy = req.body.isLegacy;
  if (req.body.manualCustomerName !== undefined) updates.manual_customer_name = req.body.manualCustomerName;
  if (req.body.manualCustomerPhone !== undefined) updates.manual_customer_phone = req.body.manualCustomerPhone;
  if (req.body.manualCustomerAddress !== undefined) updates.manual_customer_address = req.body.manualCustomerAddress;
  if (req.body.manualInvoiceNumber !== undefined) updates.manual_invoice_number = req.body.manualInvoiceNumber;
  if (req.body.customerId !== undefined && !req.body.isLegacy) updates.customer_id = req.body.customerId || null;
  if (req.body.invoiceId !== undefined && !req.body.isLegacy) updates.invoice_id = req.body.invoiceId || null;
...
app.get('/api/dashboard/summary', authenticate, async (req, res) => {
  try {
    ...
    const [
      ...
      supabase.from('invoices').select('final_amount, pos_cash_advance').eq('invoice_type', 'mastercard').eq('status', 'active'),
      supabase.from('products').select('*').eq('is_deleted', false),
      supabase.from('audit_logs').select('created_at').eq('action', 'تصدير نسخة احتياطية').order('created_at', { ascending: false }).limit(1),
    ...
    const totalMasterCard = (bankRes.data || []).reduce((acc, curr) => acc + ((parseFloat(curr.final_amount) || 0) - (parseFloat(curr.pos_cash_advance) || 0)), 0);
...
// ==================== EMPLOYEE ACCOUNTING (HR — مستقل) ====================

// Helper: calculate working days between two dates (excluding absences)
const calcWorkingDays = (startDate: string, endDate: string, absenceDates: string[]): number => {
  ...
  while (cur <= end) {
    if (cur.getDay() !== 5) { // Skip Fridays
      const dateStr = cur.toISOString().split('T')[0];
      if (!absenceSet.has(dateStr)) days++;
    }
...
// Helper: calculate expected completion date factoring in Fridays and absences
const calcExpectedCompletion = (startDate: string, repaymentDays: number, absenceDates: string[]): string => {
  if (repaymentDays <= 0) return startDate;
  const start = new Date(startDate);
  if (isNaN(start.getTime())) return startDate;
  const absenceSet = new Set(absenceDates);
  
  let daysLeft = repaymentDays;
  const cur = new Date(start);
...
app.get('/api/employee-accounting/employees', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const { data, error } = await supabase.from('employees_hr').select('*').order('id', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json((data || []).map((e: any) => ({
    ...
    status: e.status, hireDate: e.hire_date, notes: e.notes,
    createdAt: e.created_at
  })));
});

app.post('/api/employee-accounting/employees', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const { name, position, phone, monthlySalary, dailyWage, hireDate, notes } = req.body;
  if (!name) return res.status(400).json({ error: 'اسم الموظف مطلوب' });

  const { data, error } = await supabase.from('employees_hr').insert({
    name, position: position || '', phone: phone || '',
    monthly_salary: parseFloat(monthlySalary) || 0,
    daily_wage: parseFloat(dailyWage) || 0,
    status: 'active', hire_date: hireDate || null,
    notes: notes || '', created_by: req.user.name
  }).select().single();

  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'إضافة موظف HR', name, getClientIp(req));
  res.status(201).json({ id: data.id, name: data.name, position: data.position, monthlySalary: parseFloat(data.monthly_salary) || 0, dailyWage: parseFloat(data.daily_wage) || 0, status: data.status });
});

app.put('/api/employee-accounting/employees/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.edit) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const { name, position, phone, monthlySalary, dailyWage, hireDate, notes, status } = req.body;
  const updates: any = {};
  if (name) updates.name = name;
  if (position !== undefined) updates.position = position;
  if (phone !== undefined) updates.phone = phone;
  if (monthlySalary !== undefined) updates.monthly_salary = parseFloat(monthlySalary) || 0;
  if (dailyWage !== undefined) updates.daily_wage = parseFloat(dailyWage) || 0;
  if (hireDate !== undefined) updates.hire_date = hireDate || null;
  if (notes !== undefined) updates.notes = notes;
  if (status) updates.status = status;

  const { data, error } = await supabase.from('employees_hr').update(updates).eq('id', id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'تعديل موظف HR', `ID: ${id}`, getClientIp(req));
  res.json({ id: data.id, name: data.name, position: data.position, monthlySalary: parseFloat(data.monthly_salary) || 0, dailyWage: parseFloat(data.daily_wage) || 0, status: data.status });
});

app.delete('/api/employee-accounting/employees/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const { error } = await supabase.from('employees_hr').delete().eq('id', id);
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'حذف موظف HR', `ID: ${id}`, getClientIp(req));
  res.json({ success: true });
});

// ── Dashboard summary ─────────────────────────────────────────────────────────

app.get('/api/employee-accounting/summary', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });

  const [advRes, txRes, empRes] = await Promise.all([
    supabase.from('employee_advances').select('amount, remaining, status'),
    supabase.from('employee_transactions').select('type, amount'),
    supabase.from('employees_hr').select('monthly_salary').eq('status', 'active')
  ]);
  ...
  const totalSalaries = employees.reduce((s: number, e: any) => s + (parseFloat(e.monthly_salary) || 0), 0);
  const activeAdvances = advances.filter((a: any) => a.status === 'active');
  const completedAdvances = advances.filter((a: any) => a.status === 'completed');
  const totalOutstanding = activeAdvances.reduce((s: number, a: any) => s + (parseFloat(a.remaining) || 0), 0);
  const totalDeductions = transactions.filter((t: any) => t.type === 'deduction').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);
  const totalBonuses = transactions.filter((t: any) => t.type === 'bonus').reduce((s: number, t: any) => s + (parseFloat(t.amount) || 0), 0);

  res.json({ totalSalaries, activeAdvancesCount: activeAdvances.length, totalOutstanding, completedAdvancesCount: completedAdvances.length, totalDeductions, totalBonuses });
});

// ── Advances ──────────────────────────────────────────────────────────────────

app.get('/api/employee-accounting/advances', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const empId = req.query.employeeId;
  let query = supabase.from('employee_advances').select('*').order('created_at', { ascending: false });
  if (empId) query = query.eq('employee_id', parseInt(empId as string));

  const { data, error } = await query;
  if (error) return res.status(500).json({ error: error.message });

  const today = new Date().toISOString().split('T')[0];
  const absRes = await supabase.from('employee_absences').select('employee_id, date');
  const absences = absRes.data || [];

  const result = await Promise.all((data || []).map(async (adv: any) => {
    if (adv.status === 'active') {
      const empAbsences = absences.filter((a: any) => a.employee_id === adv.employee_id).map((a: any) => a.date);
      const workDays = calcWorkingDays(adv.advance_date, today, empAbsences);
      const manualRepaid = parseFloat(adv.manual_repaid) || 0;
      const repaid = Math.min((workDays * (parseFloat(adv.daily_wage) || 0)) + manualRepaid, parseFloat(adv.amount) || 0);
      const remaining = Math.max(0, (parseFloat(adv.amount) || 0) - repaid);
      const newStatus = remaining <= 0 ? 'completed' : 'active';
      
      const effectiveRepaymentDays = Math.max(0, adv.repayment_days - (manualRepaid / (parseFloat(adv.daily_wage) || 1)));
      const expectedCompletion = calcExpectedCompletion(adv.advance_date, Math.ceil(effectiveRepaymentDays), empAbsences);

      if (Math.abs(repaid - (parseFloat(adv.amount_repaid) || 0)) > 1 || newStatus !== adv.status || expectedCompletion !== adv.expected_completion) {
        await supabase.from('employee_advances').update({ amount_repaid: repaid, remaining, status: newStatus, expected_completion: expectedCompletion }).eq('id', adv.id);
        return { ...adv, amount_repaid: repaid, remaining, status: newStatus, expected_completion: expectedCompletion };
      }
    }
    return adv;
  }));

  res.json(result.map((a: any) => ({
    id: a.id, employeeId: a.employee_id, employeeName: a.employee_name,
    amount: parseFloat(a.amount) || 0, dailyWage: parseFloat(a.daily_wage) || 0,
    repaymentDays: a.repayment_days, advanceDate: a.advance_date,
    expectedCompletion: a.expected_completion, amountRepaid: parseFloat(a.amount_repaid) || 0,
    remaining: parseFloat(a.remaining) || 0, status: a.status, manualRepaid: parseFloat(a.manual_repaid) || 0,
    notes: a.notes, createdBy: a.created_by, createdAt: a.created_at
  })));
});

// Repay Advance manually
app.post('/api/employee-accounting/advances/:id/repay', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.edit) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const { amount, date, notes } = req.body;
  if (!amount || amount <= 0) return res.status(400).json({ error: 'المبلغ غير صحيح' });

  const { data: adv } = await supabase.from('employee_advances').select('*').eq('id', id).single();
  if (!adv) return res.status(404).json({ error: 'السلفة غير موجودة' });

  const newManualRepaid = (parseFloat(adv.manual_repaid) || 0) + parseFloat(amount);
  const { error } = await supabase.from('employee_advances').update({ manual_repaid: newManualRepaid }).eq('id', id);
  if (error) return res.status(500).json({ error: error.message });

  // Add a transaction record
  await supabase.from('employee_transactions').insert({
    employee_id: adv.employee_id, employee_name: adv.employee_name,
    type: 'advance_repayment', amount: parseFloat(amount), date: date || new Date().toISOString().split('T')[0],
    notes: notes || 'تسديد يدوي من السلفة',
    remaining_advance: Math.max(0, parseFloat(adv.remaining) - parseFloat(amount)),
    created_by: req.user.name
  });

  await logAudit(req.user.name, 'تسديد سلفة', `${adv.employee_name} بمبلغ: ${amount}`, getClientIp(req));
  res.json({ success: true, manualRepaid: newManualRepaid });
});

app.post('/api/employee-accounting/advances', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const { employeeId, amount, advanceDate, notes } = req.body;
  if (!employeeId || !amount || !advanceDate) return res.status(400).json({ error: 'الموظف والمبلغ والتاريخ مطلوبة' });

  const { data: emp } = await supabase.from('employees_hr').select('id,name,daily_wage').eq('id', employeeId).single();
  if (!emp) return res.status(404).json({ error: 'الموظف غير موجود' });

  const dailyWage = parseFloat(emp.daily_wage) || 0;
  const advanceAmount = parseFloat(amount);
  const repaymentDays = dailyWage > 0 ? Math.ceil(advanceAmount / dailyWage) : 0;
  
  // Calculate expected completion considering existing absences (though none might be inside the advance period yet)
  const absRes = await supabase.from('employee_absences').select('date').eq('employee_id', employeeId);
  const absences = (absRes.data || []).map((a: any) => a.date);
  const completionDateStr = calcExpectedCompletion(advanceDate, repaymentDays, absences);

  const { data, error } = await supabase.from('employee_advances').insert({
    employee_id: employeeId, employee_name: emp.name,
    amount: advanceAmount, daily_wage: dailyWage, repayment_days: repaymentDays,
    advance_date: advanceDate, expected_completion: completionDateStr,
    amount_repaid: 0, remaining: advanceAmount, status: 'active',
    notes: notes || '', created_by: req.user.name
  }).select().single();
  ...
  await supabase.from('employee_transactions').insert({
    employee_id: employeeId, employee_name: emp.name, type: 'advance',
    amount: advanceAmount, date: advanceDate, notes: notes || '',
    remaining_advance: advanceAmount, created_by: req.user.name
  });

  await logAudit(req.user.name, 'إضافة سلفة', `${emp.name}: ${advanceAmount}`, getClientIp(req));
  res.status(201).json({ id: data.id, employeeId, employeeName: emp.name, amount: advanceAmount, dailyWage, repaymentDays, advanceDate, expectedCompletion: completionDateStr, remaining: advanceAmount, status: 'active' });
});

app.put('/api/employee-accounting/advances/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.edit) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  ...
  if (notes !== undefined) updates.notes = notes;
  if (status) updates.status = status;
  const { data, error } = await supabase.from('employee_advances').update(updates).eq('id', id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.delete('/api/employee-accounting/advances/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
  const { error } = await supabase.from('employee_advances').delete().eq('id', id);
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, 'حذف سلفة', `ID: ${id}`, getClientIp(req));
  res.json({ success: true });
});

// ── Transactions ──────────────────────────────────────────────────────────────

app.get('/api/employee-accounting/transactions', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const empId = req.query.employeeId;
  let query = supabase.from('employee_transactions').select('*').order('date', { ascending: false });
  if (empId) query = query.eq('employee_id', parseInt(empId as string));
  const { data, error } = await query;
  if (error) return res.status(500).json({ error: error.message });
  res.json((data || []).map((t: any) => ({
    id: t.id, employeeId: t.employee_id, employeeName: t.employee_name,
    type: t.type, amount: parseFloat(t.amount) || 0, date: t.date,
    notes: t.notes, remainingAdvance: parseFloat(t.remaining_advance) || 0,
    createdBy: t.created_by, createdAt: t.created_at
  })));
});

app.post('/api/employee-accounting/transactions', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const { employeeId, type, amount, date, notes } = req.body;
  if (!employeeId || !type || !amount || !date) return res.status(400).json({ error: 'البيانات الأساسية مطلوبة' });

  const { data: emp } = await supabase.from('employees_hr').select('id,name').eq('id', employeeId).single();
  if (!emp) return res.status(404).json({ error: 'الموظف غير موجود' });

  const { data: activeAdv } = await supabase.from('employee_advances')
    .select('remaining').eq('employee_id', employeeId).eq('status', 'active').order('created_at').limit(1).single();
  const remainingAdvance = activeAdv ? parseFloat(activeAdv.remaining) || 0 : 0;

  const { data, error } = await supabase.from('employee_transactions').insert({
    employee_id: employeeId, employee_name: emp.name, type,
    amount: parseFloat(amount), date, notes: notes || '',
    remaining_advance: remainingAdvance, created_by: req.user.name
  }).select().single();

  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.user.name, `معاملة: ${type}`, `${emp.name}: ${amount}`, getClientIp(req));
  res.status(201).json({ id: data.id, employeeId, employeeName: emp.name, type, amount: parseFloat(amount), date, notes, remainingAdvance });
});

app.delete('/api/employee-accounting/transactions/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
...
app.get('/api/employee-accounting/absences', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.view) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const empId = req.query.employeeId;
  let query = supabase.from('employee_absences').select('*').order('date', { ascending: false });
  if (empId) query = query.eq('employee_id', parseInt(empId as string));
  const { data, error } = await query;
  if (error) return res.status(500).json({ error: error.message });
  res.json((data || []).map((a: any) => ({
    id: a.id, employeeId: a.employee_id, employeeName: a.employee_name,
    date: a.date, reason: a.reason, createdBy: a.created_by, createdAt: a.created_at
  })));
});

app.post('/api/employee-accounting/absences', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.create) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const { employeeId, date, reason } = req.body;
  if (!employeeId || !date) return res.status(400).json({ error: 'الموظف والتاريخ مطلوبان' });

  const { data: emp } = await supabase.from('employees_hr').select('id,name').eq('id', employeeId).single();
  if (!emp) return res.status(404).json({ error: 'الموظف غير موجود' });

  const { data, error } = await supabase.from('employee_absences').insert({
    employee_id: employeeId, employee_name: emp.name, date, reason: reason || '', created_by: req.user.name
  }).select().single();

  if (error) {
    if (error.code === '23505') return res.status(400).json({ error: 'هذا الغياب مسجل مسبقاً' });
    return res.status(500).json({ error: error.message });
  }
  res.status(201).json({ id: data.id, employeeId, employeeName: emp.name, date, reason });
});

// ── Inverter Exchanges & Loans ───────────────────────────────────────────────

const INVERTER_EXCHANGES_FILE = 'inverter_exchanges.json';

const loadLocalInverterExchanges = (): any[] => {
  try {
    if (fs.existsSync(INVERTER_EXCHANGES_FILE)) {
      const data = fs.readFileSync(INVERTER_EXCHANGES_FILE, 'utf-8');
      return JSON.parse(data || '[]');
    }
  } catch (e) {
    console.error('Failed to read inverter_exchanges.json:', e);
  }
  return [];
};

const saveLocalInverterExchanges = (data: any[]) => {
  try {
    fs.writeFileSync(INVERTER_EXCHANGES_FILE, JSON.stringify(data, null, 2), 'utf-8');
  } catch (e) {
    console.error('Failed to write inverter_exchanges.json:', e);
  }
};

app.get('/api/inverter-exchanges', authenticate, async (_req: any, res) => {
  try {
    const { data, error } = await supabase.from('inverter_exchanges').select('*').order('id', { ascending: false });
    if (!error && data) {
      return res.json(data.map((c: any) => ({
        ...
        deviceType: c.device_type || c.deviceType || 'inverter',
        customerInverter: c.customer_inverter,
        companyInverter: c.company_inverter,
        hasCompanyInverter: Boolean(c.has_company_inverter),
        issueDescription: c.issue_description,
        status: c.status,
        receivedDate: c.received_date,
        returnedDate: c.returned_date,
        notes: c.notes,
  ...
  res.json(loadLocalInverterExchanges().map(c => ({
    ...c,
    deviceType: c.deviceType || 'inverter'
  })));
});

app.post('/api/inverter-exchanges', authenticate, async (req: any, res) => {
  const {
    customerId, archiveId, customerName, customerPhone, customerAddress,
    deviceType, customerInverter, companyInverter, hasCompanyInverter, issueDescription,
    status, receivedDate, notes
  } = req.body;

  if (!customerName || !customerInverter || !issueDescription) {
    return res.status(400).json({ error: 'يرجى إدخال اسم العميل، بيانات وسريال الجهاز وتوصيف الخلل' });
  }
  ...
  const record = {
    ...
    archiveId: archiveId ? parseInt(archiveId) : null,
    customerName: customerName.trim(),
    customerPhone: (customerPhone || '').trim(),
    customerAddress: (customerAddress || '').trim(),
    deviceType: deviceType || 'inverter',
    customerInverter: customerInverter.trim(),
    companyInverter: (companyInverter || '').trim(),
    hasCompanyInverter: Boolean(hasCompanyInverter),
    issueDescription: issueDescription.trim(),
    status: status || (hasCompanyInverter ? 'loaned' : 'no_loan_repairing'),
  ...
  try {
    const { data: created, error } = await supabase.from('inverter_exchanges').insert({
      ...
      customer_inverter: record.customerInverter,
      company_inverter: record.companyInverter,
      has_company_inverter: record.hasCompanyInverter,
    ...
    if (!error && created) {
      await logAudit(req.user.name, 'تسجيل تبديل جهاز بديل', `${record.deviceType}: ${record.customerInverter} - ${record.customerName}`, getClientIp(req));
      return res.status(201).json({
        id: created.id, customerId: created.customer_id, archiveId: created.archive_id,
        customerName: created.customer_name, customerPhone: created.customer_phone, customerAddress: created.customer_address,
        deviceType: created.device_type || record.deviceType, customerInverter: created.customer_inverter, companyInverter: created.company_inverter,
        hasCompanyInverter: Boolean(created.has_company_inverter), issueDescription: created.issue_description,
        status: created.status, receivedDate: created.received_date, returnedDate: created.returned_date,
        notes: created.notes, createdBy: created.created_by, createdAt: created.created_at
      });
    }
  } catch (e) {}

  // Fallback to local storage
  const localList = loadLocalInverterExchanges();
  localList.unshift(record);
  saveLocalInverterExchanges(localList);

  await logAudit(req.user.name, 'تسجيل تبديل جهاز بديل', `${record.deviceType}: ${record.customerInverter} - ${record.customerName}`, getClientIp(req));
  res.status(201).json(record);
});

app.put('/api/inverter-exchanges/:id', authenticate, async (req: any, res) => {
  const id = parseInt(req.params.id);
  const { status, returnedDate, notes, companyInverter, customerInverter, issueDescription } = req.body;

  const updates: any = {};
  if (status) updates.status = status;
  if (returnedDate !== undefined) updates.returned_date = returnedDate;
  if (notes !== undefined) updates.notes = notes;
  if (companyInverter !== undefined) updates.company_inverter = companyInverter;
  if (customerInverter !== undefined) updates.customer_inverter = customerInverter;
  if (issueDescription !== undefined) updates.issue_description = issueDescription;

  if (status === 'completed' && !updates.returned_date) {
    updates.returned_date = new Date().toISOString().split('T')[0];
  }

  try {
    const { data: updated, error } = await supabase
      .from('inverter_exchanges')
      .update(updates)
      .eq('id', id)
      .select()
      .single();
    ...
    if (!error && updated) {
      await logAudit(req.user.name, 'تحديث حالة تبديل انفيرتر', `ID: ${id} - ${status}`, getClientIp(req));
      return res.json({
        id: updated.id, customerId: updated.customer_id, archiveId: updated.archive_id,
        customerName: updated.customer_name, customerPhone: updated.customer_phone, customerAddress: updated.customer_address,
        customerInverter: updated.customer_inverter, companyInverter: updated.company_inverter,
        hasCompanyInverter: Boolean(updated.has_company_inverter), issueDescription: updated.issue_description,
        status: updated.status, receivedDate: updated.received_date, returnedDate: updated.returned_date,
        notes: updated.notes, createdBy: updated.created_by, createdAt: updated.created_at
      });
    }
  } catch (e) {}
  ...
  if (index >= 0) {
    if (status) localList[index].status = status;
    if (returnedDate !== undefined) localList[index].returnedDate = returnedDate;
    if (status === 'completed' && !localList[index].returnedDate) {
      localList[index].returnedDate = new Date().toISOString().split('T')[0];
    }
    if (notes !== undefined) localList[index].notes = notes;
    if (companyInverter !== undefined) localList[index].companyInverter = companyInverter;
    if (customerInverter !== undefined) localList[index].customerInverter = customerInverter;
    if (issueDescription !== undefined) localList[index].issueDescription = issueDescription;
    
    saveLocalInverterExchanges(localList);
    await logAudit(req.user.name, 'تحديث حالة تبديل انفيرتر', `ID: ${id} - ${status}`, getClientIp(req));
    return res.json(localList[index]);
  }

  res.status(404).json({ error: 'سجل التبديل غير موجود' });
});

app.delete('/api/inverter-exchanges/:id', authenticate, async (req: any, res) => {
  const id = parseInt(req.params.id);

  try {
    await supabase.from('inverter_exchanges').delete().eq('id', id);
  } catch (e) {}

  const localList = loadLocalInverterExchanges().filter(c => c.id !== id);
  saveLocalInverterExchanges(localList);

  await logAudit(req.user.name, 'حذف سجل تبديل انفيرتر', `ID: ${id}`, getClientIp(req));
  res.json({ success: true });
});
