...
const logAudit = async (user: string, action: string, affectedRecord: string, ip: string) => {
  ...
  if (error) {
    ...
    require('fs').appendFileSync('audit_error.log', new Date().toISOString() + ' ' + action + ' ' + JSON.stringify(error) + '\n');
  }
};
...
const normalizePermissions = (rawPerm: any) => {
  ...
  const ALL_KEYS: PermissionKey[] = [
    ...
    'reports', 'employees', 'settings', 'auditLogs', 'backups', 'contracts', 'employeeAccounting'
  ];
...
app.post('/api/archive', authenticate, async (req: any, res) => {
  ...
  const { customerName, installationDate, systemSize, customerPhone, inverterSize, batteriesCount, panelsCount, installationLocation, notes } = req.body;
...
function mapFault(r: any) {
  return {
    ...
    photos: r.photos, notes: r.notes, status: r.status, createdDate: r.created_at || r.created_date,
    archiveId: r.archive_id
  };
}
...
app.delete('/api/employee-accounting/absences/:id', authenticate, async (req: any, res) => {
  if (!req.user.permissions.employeeAccounting?.delete) return res.status(403).json({ error: 'ليس لديك صلاحية' });
  const id = parseInt(req.params.id);
