...
export default function ContractsScreen({ permissions }: ContractsScreenProps) {
  ...
  const [form, setForm] = useState({
    customerId: '',
    invoiceId: '',
    contractDate: new Date().toISOString().slice(0, 10), // Default to today
    systemType: '',
    panelCount: 0,
    panelWattage: '',
    batteryCount: 0,
  ...
  return (
    <div className="flex flex-col h-full bg-slate-200/50 -mx-4 md:-mx-8 -my-4 md:-my-8 print:m-0 print:bg-white print:static print:overflow-visible print:h-auto overflow-hidden relative z-50">
      ...
      {/* Print Controls Bar */}
      <div className="bg-slate-900 p-4 shadow-xl flex justify-between items-center print:hidden sticky top-0 z-50 rounded-b-3xl overflow-x-auto">
        <div className="flex items-center gap-3">
          <button
            onClick={handlePrint}
            className="px-6 py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white font-black rounded-xl flex items-center gap-2 shadow-lg active:scale-95 transition shrink-0"
          >
          ...
          <button
            ...
            className="px-6 py-2.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl flex items-center gap-2 transition shrink-0"
          >
